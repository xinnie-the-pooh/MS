...
#define DEF_CMD_SND_DELAY  0
#define MAX_CMD_SND_DELAY  500
...
#define DEVMINCFG_VERSION       0x0020
...
#define DEVMINCFG_VERSION_OLD   0x0010

typedef struct  tagDEVMINICFG_OLD  {
    WORD  wVersion;
    WORD  wWaitBong;             // DevCfgHdr
...
} DEVMINICFG_OLD, *PDEVMINICFG_OLD;

#define MIN_DEVCONFIG_SIZE (sizeof(DEVMINICFG_OLD)-(DIAL_MODIFIER_LEN*sizeof(WCHAR)))
...
typedef struct __LineDev   {
    ...
    DWORD        dwMaxCmd;
    DWORD        dwCmdSndDelay;          // Time in milliseconds to wait before sending a response to modem
