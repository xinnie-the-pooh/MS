    ...
    ) 
{
    ...
    if (dwPhysBase < 0x20000000 && ((dwPhysBase + dwSize) < 0x20000000)) {
        ...
        // itself (KSEG1)
        //
        return Phys2VirtUC(PA2PFN(dwPhysBase));
    } else {
        //
        // Check OEM defined range.
        //
        if (dwStaticMapBase == 0 && dwStaticMapLength == 0) {
            //
            // No range has been defined yet. Allow this call to define the
            // range that can be mapped (first come, first served)
            //
            if (dwSize > NK_IO_SIZE) {
                return NULL;
            } 

            dwStaticMapBase = dwPhysBase;
            dwStaticMapLength = dwSize;

            return (LPVOID) (NK_IO_BASE);
        }

        if ((dwPhysBase >= dwStaticMapBase) && 
            (dwPhysBase + dwSize < dwStaticMapBase + dwStaticMapLength)) {
            
            return (LPVOID) (dwPhysBase - dwStaticMapBase + NK_IO_BASE);
    ...
    ) 
{
    ...
    memcpy (pbcmdline - ALIGNSTK (buflen2), buf2, buflen2);
    KPlpvTls = pTh->tlsPtr;
    ...
    ) 
{
    ...
    pTh->ctx.IntSp = (DWORD)pTh->tlsPtr
                            - SIZE_PRETLS
                            - ALIGNSTK(buflen)
                            - ALIGNSTK(buflen2)
							- 8*REG_SIZE;
    ...
    pTh->ctx.IntA1 = (long)p1;
    pTh->ctx.IntA2 = (long)p2;
#else
    pTh->ctx.IntA1 = p1;
    pTh->ctx.IntA2 = p2;
#endif
    pTh->ctx.IntA3 = (DWORD) pTh->tlsPtr - SIZE_PRETLS - ALIGNSTK(buflen);
    ((REG_TYPE*)pTh->ctx.IntSp)[4] = p4;
    pTh->ctx.IntK0 = 0;
    pTh->ctx.IntK1 = 0;
    pTh->ctx.IntRa = 0;
    pTh->ctx.Fir = (ULONG)lpBase;
    ...
    ) 
{
    ...
    if (IsInPwrHdlr ()) {
        ...
        NKDbgPrintfW(L"Exception %03x Thread=%8.8lx(%8.8lx) Proc=%8.8lx '%s'\r\n",
            cause.XCODE, pCurThread,pth, hCurProc, pCurProc->lpszProcName ? pCurProc->lpszProcName : L"");
        NKDbgPrintfW(L"AKY=%8.8lx PC=%8.8lx RA=%8.8lx BVA=%8.8lx SP=%8.8lx\r\n",
            pCurThread->aky, pth->ctx.Fir, pth->ctx.IntRa, badVAddr, pth->ctx.IntSp);
        lpNKHaltSystem ();
        FakeNKHaltSystem ();
    }
    ...
    if ((KERN_TRUST_FULL != pth->pProc->bTrustLevel)    // not fully trusted
        ...
        pth->ctx.IntSp = FixTlsAndSP (pth, !fTlsOk);
        pexi = (struct ExcInfo *)((pth->ctx.IntSp & ~63) - ALIGNSTK (sizeof(CALLSTACK)));
