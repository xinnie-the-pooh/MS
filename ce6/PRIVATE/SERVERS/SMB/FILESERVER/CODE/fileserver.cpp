                                    ...
                                    UINT *puiUsed)
{
    ...
    if(TRUE == pMyConnection->SupportsUnicode(pSMB->pInSMB)) {
        ...
        if(FAILED(pTokenizer->GetUnicodeString(&pFileName))) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Trans2_Query_Path_Information -- couldnt find filename!!"));
            dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
            goto Done;
        }
        ...
        if(0x04 != *pFileName) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler:  SMB_Trans2_Query_Path_Information -- invalid string type!"));
            dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
            goto Done;
        }
                         ...
                         UINT *puiUsed)
{
    ...
    USHORT usSID = 0xFFFF;
    SMB_FIND_FILE_BOTH_DIRECTORY_INFO_STRUCT *pBothInfoPrevFile = NULL;
    SMB_FIND_FILE_FULL_DIRECTORY_INFO_STRUCT *pFullInfoPrevFile = NULL;
    ce::smart_ptr<ActiveConnection> pMyConnection = NULL;
        ...
        do {
            ...
            switch(pffRequest->InformationLevel) {
                ...
                case SMB_FIND_FILE_BOTH_DIRECTORY_INFO:
                {
                    ...
                    if(pBothInfoPrevFile)
                        pBothInfoPrevFile->NextEntryOffset = (UINT)pFileStruct - (UINT)pBothInfoPrevFile;
                ...
                case SMB_FIND_FILE_FULL_DIRECTORY_INFO:
                    {
                        ...
                        BYTE *pNewBlock = NULL;
                        BYTE *pFileBlob = NULL;
                        SMB_FIND_FILE_FULL_DIRECTORY_INFO_STRUCT *pFileStruct = NULL;
                        ...
                        // If we cant reserve enough memory for this block we need to start up a
                        //   FindNext setup
                        if(FAILED(MYRAPIBuilder.ReserveBlock(uiFileBlobSize + sizeof(SMB_FIND_FILE_FULL_DIRECTORY_INFO_STRUCT), (BYTE**)&pFileStruct, sizeof(DWORD)))) {
                            dwRet = 0;
                            pffResponse->EndOfSearch = 0;
                            LocalFree(pFileBlob);
                            goto SendOK;
                        ...
                        pFileStruct->FileIndex = 0;
                        pFileStruct->NextEntryOffset = 0;
                        if(pFullInfoPrevFile)
                            pFullInfoPrevFile->NextEntryOffset = (UINT)pFileStruct - (UINT)pFullInfoPrevFile;
                        pFileStruct->CreationTime.LowPart = pwfd->ftCreationTime.dwLowDateTime;
                        pFileStruct->CreationTime.HighPart = pwfd->ftCreationTime.dwHighDateTime;
                        pFileStruct->LastAccessTime.LowPart = pwfd->ftLastAccessTime.dwLowDateTime;
                        pFileStruct->LastAccessTime.HighPart = pwfd->ftLastAccessTime.dwHighDateTime;
                                   ...
                                   SMB_PACKET *pSMB)
{
    ...
    if(_pRawResponse->uiDataSize < sizeof(SMB_COM_GENERIC_RESPONSE)) {
       TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_NT_Cancel -- not enough memory for response!"));
       dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
       goto Done;
    }
                                   ...
                                   SMB_PACKET *pSMB)
{
    ...
    if(_pRawRequest->uiDataSize < sizeof(SMB_TRANS2_FIND_CLOSE_CLIENT_REQUEST)) {
       TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_FindClose2 -- not enough memory for request!"));
       dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
       goto Done;
    }
    if(_pRawResponse->uiDataSize < sizeof(SMB_COM_GENERIC_RESPONSE)) {
       TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_FindClose2 -- not enough memory for response!"));
       dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
       goto Done;
    }
...
DWORD SMB_FILE::SMB_Com_NT_TRASACT(SMB_PROCESS_CMD *_pRawRequest, SMB_PROCESS_CMD *_pRawResponse, UINT *puiUsed, SMB_PACKET *pSMB)
{
    ...
    if(_pRawRequest->uiDataSize < sizeof(SMB_COM_NT_TRANSACTION_CLIENT_REQUEST) ||
       _pRawRequest->uiDataSize < sizeof(SMB_COM_NT_TRANSACTION_CLIENT_REQUEST) +  (sizeof(WORD)*pRequest->SetupCount)) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_NT_TRASACT -- not enough memory for request!"));
        dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
        goto Done;
    }
    if(_pRawResponse->uiDataSize < sizeof(SMB_COM_NT_TRANSACTION_SERVER_RESPONSE)) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_NT_TRASACT -- not enough memory for response!"));
        dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
        goto Done;
    }
...
HRESULT RetrieveLock(SMB_LARGELOCK_RANGE *pOut, BOOL fUsingLargeLock, StringTokenizer &RequestTokenizer)
{
    ...
    if(fUsingLargeLock) {
         ...
         if(FAILED(RequestTokenizer.GetByteArray((BYTE **)&pRange, sizeof(SMB_LARGELOCK_RANGE)))) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_LockX -- problem getting range from packet!!"));
            goto Done;
         }
                               ...
                               SMB_PACKET *pSMB)
{
    ...
    if(_pRawRequest->uiDataSize < sizeof(SMB_LOCKX_CLIENT_REQUEST)) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_LockX -- not enough memory for request!"));
        dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
        goto Done;
    }
    if(_pRawResponse->uiDataSize < sizeof(SMB_LOCKX_SERVER_RESPONSE)) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_LockX -- not enough memory for response!"));
        dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
        goto Done;
    }
    ...
    // Find a share state
    if(FAILED(pMyConnection->FindTIDState(_pRawRequest->pSMBHeader->Tid, pTIDState, SEC_READ)) || !pTIDState) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_LockX -- couldnt find share state!!"));
        dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
        goto Done;
    }
    ...
    for(uiCnt = 0; uiCnt < pRequest->NumLocks; uiCnt++) {
         ...
         if(FAILED(RetrieveLock(&RangeLock, fUsingLargeLock, RequestTokenizer))) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_LockX -- cant get lock from packet!!!"));
         ...
         if(FAILED(pTIDState->IsLocked(pRequest->FileID, &RangeLock, &fLocked)) ||
            TRUE == fLocked) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_LockX -- range already locked!!!"));
            dwRet = ERROR_CODE(STATUS_WAS_UNLOCKED);
            goto Done;
         }
    }
    ...
    for(uiCnt = 0; uiCnt < pRequest->NumUnlocks; uiCnt++) {
         ...
         if(FAILED(pTIDState->IsLocked(pRequest->FileID, &RangeLock, &fLocked)) ||
            FALSE == fLocked) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_LockX -- unlock requested on range not already locked: FID: %d!!!",pRequest->FileID));
            ASSERT(FALSE);
         }
    }
    ...
    for(uiCnt = 0; uiCnt < pRequest->NumLocks; uiCnt ++) {
         ...
         if(FAILED(hr = pTIDState->Lock(pRequest->FileID, &RangeLock))) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_LockX -- cant lock range!!!"));
            dwRet = ConvertHRToError(hr, pSMB);
            goto Done;
         }
    }
    ...
    for(uiCnt = 0; uiCnt < pRequest->NumUnlocks; uiCnt ++) {
         ...
         if(FAILED(pTIDState->UnLock(pRequest->FileID, &RangeLock))) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_LockX -- can't unlock range!!!"));
    ...
    if(pRequest->LockType & BREAK_OPLOCK) {
        if(FAILED(pTIDState->BreakOpLock(pRequest->FileID))) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_LockX cant break oplock for FID:%d", pRequest->FileID));
            ASSERT(FALSE);
        } else {
                                     ...
                                     SMB_PACKET *pSMB)
{
    ...
    if(FAILED(hr = pMyConnection->FindTIDState(_pRawRequest->pSMBHeader->Tid, pTIDState, SEC_WRITE)) || !pTIDState) {
        if(E_ACCESSDENIED == hr) {
            ...
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_MakeDirectory -- couldnt find share state!!"));
    ...
    if(_pRawRequest->uiDataSize < sizeof(SMB_CREATE_DIRECTORY_CLIENT_REQUEST) + pRequest->ByteCount) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_MakeDirectory -- not enough memory for request!"));
        dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
        goto Done;
    }
    if(_pRawResponse->uiDataSize < sizeof(SMB_COM_GENERIC_RESPONSE)) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_MakeDirectory -- not enough memory for response!"));
        dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
        goto Done;
    }
    ...
    if(TRUE == pMyConnection->SupportsUnicode(pSMB->pInSMB)) {
        ...
        if(FAILED(RequestTokenizer.GetUnicodeString(&pDirectoryName))) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_MakeDirectory -- couldnt find filename!!"));
            dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
            goto Done;
        }
        ...
        if(0x04 != *pDirectoryName) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler:  SMB_Com_MakeDirectory -- invalid string type!"));
            dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
            goto Done;
        }
    ...
    if(FAILED(pTIDState->GetShare()->IsValidPath(FullPath.GetString()))) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_MakeDirectory -- error filename has invalid character!"));
    ...
    TRACEMSG(ZONE_FILES, (L"SMBSRV-PktHandler: SMB_Com_MakeDirectory -- creating directory %s", FullPath.GetString()));
    if(0 == CreateDirectory(FullPath.GetString(), 0)) {
        dwRet = ConvertGLEToError(GetLastError(), pSMB);
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_MakeDirectory -- error CreateDirectory() on directory %s failed with %d!", FullPath.GetString(), GetLastError()));
        goto Done;
    }
                                  ...
                                  SMB_PACKET *pSMB)
{
    ...
    if(FAILED(hr = pMyConnection->FindTIDState(_pRawRequest->pSMBHeader->Tid, pTIDState, SEC_WRITE)) || !pTIDState) {
        if(E_ACCESSDENIED == hr) {
            ...
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_DeleteFile -- couldnt find share state!!"));
            dwRet = SMB_ERR(ERRSRV, ERRerror);
            goto Done;
        }
    }
    ...
    if(_pRawRequest->uiDataSize < sizeof(SMB_DELETE_FILE_CLIENT_REQUEST) + pRequest->ByteCount) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_DeleteFile -- not enough memory for request!"));
        dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
        goto Done;
    }
    if(_pRawResponse->uiDataSize < sizeof(SMB_DELETE_FILE_SERVER_RESPONSE)) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_DeleteFile -- not enough memory for response!"));
        dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
        goto Done;
    }
    ...
    // make sure the request doesnt have any unexpected characters
    if(FAILED(pTIDState->GetShare()->IsValidPath(FullPath.GetString()))) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_DeleteFile -- error filename has invalid character!"));
        dwRet = ERROR_CODE(STATUS_OBJECT_NAME_NOT_FOUND);
        goto Done;
    }
    ...
    if(FAILED(SMB_Globals::g_pAbstractFileSystem->AFSDeleteFile(FullPath.GetString()))) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_DeleteFile -- error DeleteFile() on filename %s failed with %d!", FullPath.GetString(), GetLastError()));
        dwRet = ConvertGLEToError(GetLastError(), pSMB);
        goto Done;
    }
                                    ...
                                    SMB_PACKET *pSMB)
{
    ...
    if(_pRawRequest->uiDataSize < sizeof(SMB_RENAME_FILE_CLIENT_REQUEST) + pRequest->ByteCount) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_RenameFile -- not enough memory for request!"));
        dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
        goto Done;
    }
    if(_pRawResponse->uiDataSize < sizeof(SMB_COM_GENERIC_RESPONSE)) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_RenameFile -- not enough memory for response!"));
        dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
        goto Done;
    }
    ...
    if(FAILED(hr = pMyConnection->FindTIDState(_pRawRequest->pSMBHeader->Tid, pTIDState,SEC_WRITE)) || !pTIDState) {
        if(E_ACCESSDENIED == hr) {
            ...
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_RenameFile -- couldnt find share state!!"));
            dwRet = SMB_ERR(ERRSRV, ERRerror);
            goto Done;
        }
    }
    ...
    if(FALSE == pMyConnection->SupportsUnicode(pSMB->pInSMB)) {
        ...
        if(FAILED(RequestTokenizer.GetByte(&fileOldID)) || 0x04 != fileOldID) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_RenameFile -- improper string id on old filename!!"));
            dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
            goto Done;
        }
        if(FAILED(RequestTokenizer.GetString(&pFileOld))) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_RenameFile -- couldnt find old filename!!"));
            dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
            goto Done;
        }
        if(FAILED(RequestTokenizer.GetByte(&fileNewID)) || 0x04 != fileNewID) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_RenameFile -- improper string id on new filename!!"));
            dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
            goto Done;
        }
        if(FAILED(RequestTokenizer.GetString(&pFileNew))) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_RenameFile -- couldnt find new filename!!"));
            dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
            goto Done;
        }
    ...
    if(FAILED(pTIDState->GetShare()->IsValidPath(FullOldFile.GetString()))) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_RenameFile -- error old filename has invalid character!"));
        dwRet = ERROR_CODE(STATUS_OBJECT_NAME_NOT_FOUND);
        goto Done;
    }
    ...
    TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_Renamefile -- rename %s to %s", FullOldFile.GetString(), FullNewFile.GetString()));
    if(0 == wcscmp(FullOldFile.GetString(), FullNewFile.GetString())) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_RenameFile -- Files are the same... doing nothing %s->%s!", FullOldFile.GetString(), FullNewFile.GetString()));
    } else if(0 == MoveFile(FullOldFile.GetString(), FullNewFile.GetString())) {
           TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_RenameFile -- MoveFile() failed with %d on %s->%s!", GetLastError(), FullOldFile.GetString(), FullNewFile.GetString()));
           dwRet = ConvertGLEToError(GetLastError(), pSMB);
           goto Done;
    }
                                         ...
                                         SMB_PACKET *pSMB)
{
    ...
    *puiUsed = 0;
    ...
    if(FAILED(pTIDState->GetShare()->IsValidPath(FileString.GetString()))) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: COM_QUERY_INFO -- error filename has invalid character!"));
        goto BadFile;
    }
                                      ...
                                      SMB_PACKET *pSMB)
{
    ...
    *puiUsed = 0;
    ...
    if(FAILED(hr = pTIDState->SetFileTime(pRequest->FileID, pCreationTime, pAccessTime, pWriteTime))) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_SetInformation2 -- couldnt set file time!!"));
        dwRet = ConvertHRToError(hr, pSMB);
        goto Done;
    }
                                     ...
                                     SMB_PACKET *pSMB)
{
    ...
    *puiUsed = 0;
    ...
    if(TRUE == pMyConnection->SupportsUnicode(pSMB->pInSMB)) {
        ...
        if(FAILED(RequestTokenizer.GetUnicodeString(&pDirectoryName))) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_Com_CheckPath -- couldnt find filename!!"));
            dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
            goto Done;
        }
        FullPath.append(pDirectoryName);
        ...
        // Check that we have the right string type
        if(0x04 != *pDirectoryName) {
            TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler:  SMB_Com_CheckPath -- invalid string type!"));
            dwRet = ERROR_CODE(STATUS_INTERNAL_ERROR);
            goto Done;
        }
    ...
    if(FAILED(pMyShare->IsValidPath(FullPath.GetString()))) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler:  SMB_Com_CheckPath -- invalid string %s -- not safe!", FullPath.GetString()));
        dwRet = ERROR_CODE(STATUS_OBJECT_NAME_INVALID);
        goto Done;
    }
                                            ...
                                            UINT *puiUsed)
{
    ...
    *puiUsed = 0;
    ...
    //
    // Seek out the FID
    if(FAILED(hr = pTIDState->QueryFileInformation(pRequest->FID, &fd, NULL))) {
                 ...
                 SMBFile_OpLock_PassStruct *pdwOpLock)
{
    ...
    if(NULL == (pTIDState = this->GetTIDState())) {
        ASSERT(FALSE);
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_OpenX -- cant get TID STATE!!!!"));
        hr = E_UNEXPECTED;
        goto Done;
    }
...
//    #error handle case where these fail!
    ...
    // make sure the request doesnt have any unexpected characters
    if(FAILED(pTIDState->GetShare()->IsValidPath(FullPath.GetString()))) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_OpenX -- error filename has invalid character!"));
        hr = E_ACCESSDENIED;
        goto Done;
    }
    ...
    if(FAILED(hr = IsDirectory(FullPath.GetString(), &fIsDirectory, &fFileExists))) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_OpenX -- error cant check file as directory!! %s", FullPath.GetString()));
        ASSERT(FALSE);
        hr = E_ACCESSDENIED;
        goto Done;
    }
    if(fIsDirectory && (dwAttributes & FILE_ATTRIBUTE_CANT_BE_DIR)) {
        TRACEMSG(ZONE_ERROR, (L"SMBSRV-PktHandler: SMB_OpenX -- cant open because the file is a directory!! %s", FullPath.GetString()));
    ...
    if(fIsDirectory || (dwAttributes & FILE_ATTRIBUTE_MUST_BE_DIR)) {
        TRACEMSG(ZONE_FILES, (L"SMBSRV-PktHandler: SMB_OpenX -- opening up as directory!! %s", FullPath.GetString()));
    ...
    if(FAILED(hr = SMB_Globals::g_pAbstractFileSystem->Open(FullPath.GetString(),
                                                       ...
                                                       pdwOpLock)))
    {
        TRACEMSG(ZONE_FILES, (L"SMBSRV-PktHandler: SMB_OpenX -- error -- cant open up from abstract file system!"));
        goto Done;
    }
    m_fOpened = TRUE;
    m_dwCurrentOffset = 0;
...
//
// NOTE: this API is mainly used as an old way of figuring out
...
HRESULT
FileStream::QueryFileInformation(WIN32_FIND_DATA *fd, DWORD *pdwNumberOfLinks)
{
    PREFAST_ASSERT(NULL != SMB_Globals::g_pAbstractFileSystem);
    ce::smart_ptr<FileObject> pObject = NULL;
    HRESULT hr = E_FAIL;
    ...
    pObject->GetFileInformation(NULL, NULL, NULL, NULL, NULL, NULL, pdwNumberOfLinks, NULL, NULL);
                       ...
                       LPFILETIME lpLastWriteTime)
{
    ...
    //  else try the operation with a real GetFileTime, if that succeeds great..
    //
...
HRESULT
FileObject::GetFileInformation(DWORD* pdwFileAttributes,
                        ...
                        DWORD* pdwFileIndexLow)
{
    ...
    TRACEMSG(ZONE_FILES, (L"GetFileInformation on %s FID: %d got handle 0x%x:", FileName(), m_usFID, (UINT)m_hFile));

    // if the file is not open then just return default values
    if(GetFileInformationByHandle(m_hFile, &bhfi)) {
        if(pdwFileAttributes) *pdwFileAttributes = bhfi.dwFileAttributes;
        if(lpCreationTime) *lpCreationTime = bhfi.ftCreationTime;
        if(lpLastAccessTime) *lpLastAccessTime = bhfi.ftLastAccessTime;
        if(lpLastWriteTime) *lpLastWriteTime = bhfi.ftLastWriteTime;
        if(pdwFileSizeHigh) *pdwFileSizeHigh = bhfi.nFileSizeHigh;
        if(pdwFileSizeLow) *pdwFileSizeLow = bhfi.nFileSizeLow;
        if(pdwNumberOfLinks) *pdwNumberOfLinks = bhfi.nNumberOfLinks;
        if(pdwFileIndexHigh) *pdwFileIndexHigh = bhfi.nFileIndexHigh;
        if(pdwFileIndexLow) *pdwFileIndexLow = bhfi.nFileIndexLow;
    }
...

{
    ...
    //
    // Free up the PktHandler, and wait for our event to finish processing
    csLock.UnLock();
             ...
             SMBFile_OpLock_PassStruct *pOpLock)
{
    ...
    ASSERT(dwOpLockRequested == NT_CREATE_BATCH_OPLOCK ||
           dwOpLockRequested == NT_CREATE_NO_OPLOCK);
