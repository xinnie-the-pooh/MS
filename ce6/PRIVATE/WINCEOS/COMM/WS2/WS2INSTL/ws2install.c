...
#define NUM_IRDA_PROTOS     1   // AF_IRDA==22
	...
	PBYTE pMTable, DWORD cMTable, PDWORD pIndex) 
{
	...
	*pIndex = 0;
    ...
    cIrdastk =  (IsStackInstalled(TEXT("irdastk"), TRUE)) ? NUM_IRDA_PROTOS : 0;
    cBluetooth = IsStackInstalled(TEXT("btd"), TRUE);
    cIPv6 =     (IsStackInstalled(TEXT("tcpip6"), TRUE)) ? NUM_TCPIP6_PROTOS : 0;
	cEntries = NUM_TCPIP_PROTOS + cIrdastk + cBluetooth + cIPv6;
