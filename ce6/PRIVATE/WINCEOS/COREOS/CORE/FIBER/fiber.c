...
void MDFiberSwitch (PFIBERSTRUCT pFiber, DWORD dwThrdId, LPDWORD pTlsPtr);
...
LPVOID WINAPI ConvertThreadToFiber (LPVOID lpParameter)
{
    ...
    // allocate a fiber structure and initialize it
    if (!(pFiber = (PFIBERSTRUCT) LocalAlloc(LMEM_FIXED, sizeof(FIBERSTRUCT)))) {
        SetLastError (ERROR_NOT_ENOUGH_MEMORY);
        return NULL;
    }
    pFiber->lpParam = lpParameter;
...
VOID WINAPI SwitchToFiber (PFIBERSTRUCT pFiber)
{
    ...
    MDFiberSwitch (pFiber, dwCurId, UTlsPtr ());
}
