...
BOOL loading_shim;
BOOL g_bIsAppVerifierActive = FALSE;
...
{
    // Check the registry for a global key.
    HKEY hKey;
    DWORD dwLastErr;

    if ((BOOL) -1 == appverif_enabled)
    {
        // Preserve last error
        dwLastErr = GetLastError ();

        // Default.
        appverif_enabled = FALSE;

        if (WAIT_OBJECT_0 == WaitForAPIReady (SH_FILESYS_APIS, 0))
        {
            if (ERROR_SUCCESS == RegOpenKeyEx (HKEY_LOCAL_MACHINE, _T("ShimEngine"), 0, KEY_ALL_ACCESS, & hKey))
            {
                DWORD dwType;
                DWORD cbData;

                cbData = sizeof (BOOL);
                LONG lRet = RegQueryValueEx (hKey, _T("GlobalEnable"), NULL, & dwType, (LPBYTE) & appverif_enabled, & cbData);
        ...
        {
            appverif_enabled = TRUE;
        }

        SetLastError (dwLastErr);
    }

    return appverif_enabled;
}
    ...
    )
{
    ...
    if (pShimInfo)
    {
        ...
        if (pShimInfo->pMod)
        {
            ...
            DEBUGMSG (DBGLOADER, (_T("ERROR: '%s' is not a valid shim.\r\n"), pszShimName));
            FreeLibrary (pShimInfo->pMod->hModule);
        }
        else
        {
            DEBUGMSG (DBGLOADER, (_T("ERROR: could not load '%s'\r\n"), pszShimName));
        }
    ...
    )
{
    ...
    if (WAIT_OBJECT_0 == WaitForAPIReady (SH_FILESYS_APIS, 0))
    {
        ...
        // Apply the base shim whenever any other shims are applied.
        pShimInfoMod = pShimInfoMod ?
            MergeShimInfoLists (pShimInfoMod, LoadShim (BASE_SHIM)) :
            NULL;
    ...
    )
{
    ...
    if (process_modules == NULL)
        return NULL;


    if (pModuleInfo = (LPMODULE_INFO) LocalAlloc (LMEM_ZEROINIT, sizeof (MODULE_INFO)))
    {
    ...
    )
{
    ...
    if (!(pMod->wFlags & MF_APP_VERIFIER) || !(pMod2->wFlags & MF_APP_VERIFIER))
    {
    ...
    )
{
    ...
    DEBUGMSG (DBGLOADER, (_T("Dll list:\r\n")));
    ...
    )
{
    ...
    AddModuleInfo (pszFileName, pMod);
...
{
    if (!IsAppVerifierEnabled () || loading_shim || (pImpInfo->size == 0))
    {
        // No reason to go any farther.
        return FALSE;
    }

    if (dwBaseAddr == 0x10000)
    {
        // This is the the exe.
        TCHAR szModuleName [MAX_PATH];
        ...
        g_pProcMod = (PUSERMODULELIST) LocalAlloc (LMEM_ZEROINIT, sizeof (USERMODULELIST));
        g_pProcMod->dwBaseAddr = dwBaseAddr;

        AddModuleInfo (szModuleName, g_pProcMod);
    }
    ...
    if (pModuleInfo->pMod && (pModuleInfo->pMod->wFlags & MF_APP_VERIFIER))
    {
        ...
        )
    {
        ...
        if (fRecursive)
        {
            ...
            AppVerifierDoImports (pMod->dwBaseAddr, GetImportInfo (pMod), TRUE);
        }
    }
...
{
    if (!IsAppVerifierEnabled () || loading_shim)
    {
        // No reason to go any farther.
        return;
    }
...
BOOL AttachToRunning (PUSERMODULELIST pMod)
{
    ...
    if (pMod)
    {
        ...
        return AppVerifierDoImports (pMod->dwBaseAddr, GetImportInfo (pMod), FALSE);
    }
#endif
    ...
    )
{
    switch (dwIoControlCode)
    {
        case IOCTL_APPVERIF_ATTACH:
            return AttachToRunning ((PUSERMODULELIST) lpInBuf);
    }
