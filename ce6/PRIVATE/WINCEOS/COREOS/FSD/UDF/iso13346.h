...
//  NSRPVD_F_... - Definitions for Flags
 ...
 *
 *      Destag.destag_Ident = DESTAG_ID_NSR_ANCHOR
 *
 */
 ...
 *
 *      Destag.destag_Ident = DESTAG_ID_NSR_IMPUSE
 *
 */
...
//
//  UDF IUVD implementation use field structure.  See 2.2.7.2.
//

typedef struct _IUVD_IMP_USE  {
...
typedef struct  NSR_PART {
    ...
    UCHAR       ContentsUse[128];       // Partition Contents Use (in UDF this == Partition Header 4/14.3)
 ...
 *
 *      Destag.destag_Ident = DESTAG_ID_NSR_LVOL
 *
...
#define ISONsrLvolSize( L ) (ISONsrLvolConstantSize + (L)->MapTableLength)
...
//
//  UDF 1.50 CD UDF VAT Trailer
//

typedef struct _VAT_TRAILER {

    //  VAT entries immediately preceed the trailer.
    //
    //  UINT32  Lba of virtual sector 0
    //  UINT32  Lba of virtual sector 1
    //  ...    

    REGID       RegId;              // VAT Entity Identifier 
                                    //  = UdfVatTableIdentifier    
    ULONG       PreviousVatIcbLbn;
       
} VAT_TRAILER, *PVAT_TRAILER;
...
//////////
//  UDF Metadata Partitions are identified via a type 2 partition map of the following form.
//////////

typedef struct _PARTMAP_METADATA {

    UCHAR       Type;                   //  Partition Map Type = 2
    UCHAR       Length;                 //  Partition Map Length = 64
    UCHAR       Reserved2[2];           //  Reserved Padding
    REGID       PartID;                 //  Paritition Entity Identifier
                                        //   == UdfMetadataPartitionDomainIdentifier
    USHORT      VolSetSeq;              //  Volume Set Sequence
    USHORT      Partition;              //  Related Partition
    ULONG       FileLocation;           //  Metadata File Location (ICB Lbn in phys part)
    ULONG       MirrorFileLocation;     //  Metadata Mirror File Location (ICB Lbn...)
    ULONG       BitmapLocation;         //  Metadata Bitmap Location (ICB Lbn...)
    ULONG       AllocationUnitSize;     //  Allocation Unit for Metadata File (blocks)
    USHORT      AlignmentUnitSize;      //  Alignment Unit for Metadata File (blocks)
    UCHAR       Flags;                  //  Flags 
    UCHAR       Reserved59[5];          //  Reserved Padding

} PARTMAP_METADATA, *PPARTMAP_METADATA;

//  PARTMAP_METADATA_F_... - Definitions for PARTMAP_METADATA_Flags 

#define PARTMAP_METADATA_F_DUPLICATE        (0x01)  // Metadata Duplicate Bit
...
//
//  UDF LVID implementation use field structure.  See 2.2.6.4.
//

typedef struct _LVID_IMP_USE  {
...
//  Macros for variant portion access

#define NsrLvidImpUseOffset( I)  (sizeof( NSR_INTEG) + ((I)->PartitionCount * 2 * sizeof( ULONG)))
#define NsrLvidSize( I)          (NsrLvidImpUseOffset(I) + (I)->ImpUseLength)

#define NsrLvidFreeTable( I)     ((PULONG)((I) + 1))
#define NsrLvidSizeTable( I)     (NsrLvidFreeTable(I) + (I)->PartitionCount)
 ...
 *      (ImpUseLen-1) to the UCHAR ARRAY INDEX.
 *
 ...
 *      (ImpUseLen+nsr_fid_FileIDLen-2) to the UCHAR ARRAY INDEX.
 *
 *      The true total size of nsr_fid_s is
 *          ((38 + nsr_fid_FileIDLen + ImpUseLen) + 3) & ~3)
 *
 */
...
//  UCHAR       ImpUse[];                  // Implementation Use Area

//  nsr_fid_ImpUse has a variant length = ImpUseLen

//  UCHAR       FileID[];                  // File Identifier
...
#define ISONsrFidSize( F )      (LongAlign( sizeof( NSR_FID) + (F)->FileIDLen + (F)->ImpUseLen ))
#define NsrFidIdentifier( F)    (((PUCHAR)(F)) + sizeof( NSR_FID) + ((F)->ImpUseLen))

//  NSR_FID_F_... - Definitions for Flags (Characteristics, 4/14.4.3)
...
#define ICBTAG_FILE_T_STREAMDIR 13      // (Ecma r3).  Stream Directory
...
#define ICBTAG_F_STREAM         (0x2000)// File is referenced by Fid in a stream dir.

//
//  Flags which we preserve when we update the Icb
//

#define ICBTAG_F_PRESERVE_MASK  (ICBTAG_F_SETUID | ICBTAG_F_SETGID | ICBTAG_F_STICKY | \
                                 ICBTAG_F_STREAM | ICBTAG_F_ALLOC_MASK)

//
//  Flags which will cause us to run away from this stream.
//

#define ICBTAG_F_ILLEGAL_FLAGS  (ICBTAG_F_MULTIVERSIONS | ICBTAG_F_TRANSFORMED)
...
typedef struct  ICBEXTFILE {
    ...
    ULONGLONG   InfoLength;             // Information Length (file size)
                                        ...
                                        // THE FOLLOWING FIELD IS NEW IN THE EXT FE vs BASIC
                                        // FE.  FIELDS AFTER THIS POINT ARE DISPLACED AND SHOULD
                                        // BE ACCESSED USING THE MACROS BELOW WHEN 
                                        // MANIPULATING RAW, MAPPED, FEs

    ULONGLONG   ObjectSize;             // Object Size (Sum of InfoLengths for all streams) EXT FE ONLY
    ULONGLONG   BlocksRecorded;         // Logical Blocks Recorded
    TIMESTAMP   AccessTime;             // Last-Accessed Time
    TIMESTAMP   ModifyTime;             // Last-Modification Time
    TIMESTAMP   CreationTime;           // Creation Time                    EXT FE ONLY
    TIMESTAMP   AttributeTime;          // Last-Attribute-Change Time
    ULONG       Checkpoint;             // File Checkpoint
    ULONG       Reserved;
    LONGAD      IcbEA;                  // Extended Attribute ICB
    LONGAD      IcbStream;              // Stream Directory ICB             EXT FE ONLY
    REGID       ImpUseID;               // Implementation Use Identifier
    ULONGLONG   UniqueID;               // Unique ID
    ULONG       EALength;               // Length of Extended Attributes
    ULONG       AllocLength;            // Length of Allocation Descriptors
...
#define PFeBlocksRecorded( F)        (((F)->Destag.Ident == DESTAG_ID_NSR_FILE) ? \
...
#define PFeAttributeTime( F)        (((F)->Destag.Ident == DESTAG_ID_NSR_FILE) ? \
                                     &(F)->AttributeTime : &((PICBEXTFILE)(F))->AttributeTime)
...
#define PFeImplID( F)               (((F)->Destag.Ident == DESTAG_ID_NSR_FILE) ? \
                                     &(F)->ImpUseID : &((PICBEXTFILE)(F))->ImpUseID)

#define FeUniqueId( F)              (((F)->Destag.Ident == DESTAG_ID_NSR_FILE) ? \
                                     (F)->UniqueID : ((PICBEXTFILE)(F))->UniqueID)
                                     
#define FeCheckPoint( F)            (((F)->Destag.Ident == DESTAG_ID_NSR_FILE) ? \
                                     (F)->Checkpoint : ((PICBEXTFILE)(F))->Checkpoint)
                                     
#define FeEaLength( F)              ((((F)->Destag.Ident == DESTAG_ID_NSR_FILE) ? \
                                     (F)->EALength : ((PICBEXTFILE)(F))->EALength))

#define FeObjectSize( F)            ((PICBEXTFILE)(F))->ObjectSize
                                     
#define FeAllocLength( F)           ((((F)->Destag.Ident == DESTAG_ID_NSR_FILE) ? \
                                     (F)->AllocLength : ((PICBEXTFILE)(F))->AllocLength))

#define FeEas( F)                   Add2Ptr((F), (((F)->Destag.Ident == DESTAG_ID_NSR_FILE) ? sizeof( ICBFILE) : sizeof( ICBEXTFILE)), PUCHAR)

#define FeEasFieldOffset( F)        (((F)->Destag.Ident == DESTAG_ID_NSR_FILE) ? sizeof( ICBFILE) : sizeof( ICBEXTFILE))

#define FeIsExtended( F)            ((F)->Destag.Ident == DESTAG_ID_NSR_EXT_FILE)

#define FeCrcSize( F)               (FeIsExtended(F) ? sizeof( ICBEXTFILE) : sizeof( ICBFILE))

#define FeSize( F)                  (FeCrcSize(F) + FeEaLength( F) + FeAllocLength(F))


//  Macros for setting FE/XFE fields

#define SetFeUniqueId( F, V)        if (FeIsExtended(F)) ((PICBEXTFILE)(F))->UniqueID = (V);    else (F)->UniqueID = (V)
#define SetFeCheckPoint( F, V)      if (FeIsExtended(F)) ((PICBEXTFILE)(F))->Checkpoint = (V);  else (F)->Checkpoint = (V)
#define SetFeEaLength( F, V)        if (FeIsExtended(F)) ((PICBEXTFILE)(F))->EALength = (V);    else (F)->EALength = (V)
#define SetFeAllocLength( F, V)     if (FeIsExtended(F)) ((PICBEXTFILE)(F))->AllocLength = (V); else (F)->AllocLength = (V)
...
typedef struct  NSR_PATH {
    ...
    USHORT      CompVer;                // Path Component Version
