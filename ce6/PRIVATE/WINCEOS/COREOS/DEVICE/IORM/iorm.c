...
static void
RBMInit(DWORD dwResMax, const PDWORD pdwMask, BOOL fSet)
{
    DWORD dwSize;
...
static BOOL
RBMIsSet(DWORD dwResMax, DWORD dwElement, const PDWORD pdwMask)
{
    DWORD dwIndex;
    BOOL fIsSet;
...
static void
RBMSet(DWORD dwResMax, DWORD dwElement, const PDWORD pdwMask)
{
    DWORD dwIndex;
...
static void
RBMClear(DWORD dwResMax, DWORD dwElement, const PDWORD pdwMask)
{
    DWORD dwIndex;
        ...
        switch (dwType) {
          case REG_SZ:{
            ...
            while (ScanRange(&pos, &a[0], &a[1])) {
                BOOL fOk = IORM_ResourceRequestEx(dwResID, a[0], a[1], RREXF_REQUEST_EXCLUSIVE);
                DEBUGCHK(fOk);
            }
            ...
            break;
          case REG_DWORD: {
            BOOL fOk = IORM_ResourceRequestEx(dwResID, RegValue.dwVal, 1 , RREXF_REQUEST_EXCLUSIVE);
            DEBUGCHK(fOk);
          }
            break;
          default:
