...
const ULONGLONG AFSAPISigs[NUM_AFS_APIS] = {
        ...
        FNSIG11(DW,DW,I_WSTR,DW,DW,PTR,DW,DW,DW,I_PTR,DW), // CreateFileW
...
BOOL FSDMGR_GetVolumeInfo (MountedVolume_t* pVolume, CE_VOLUME_INFO_LEVEL InfoLevel, 
    CE_VOLUME_INFO* pInfo, DWORD* pBytesReturned)
{
    ...
    memset (&VolumeInfo, 0, sizeof (CE_VOLUME_INFO));
    ...
    return CeSafeCopyMemory (pInfo, &VolumeInfo, sizeof (CE_VOLUME_INFO));   
}
        ...
        switch (Fsctl) {
            ...
            case FSCTL_GET_VOLUME_INFO:
                if (pInBuf && pOutBuf &&
                    ...
                    (OutBufSize == sizeof(CE_VOLUME_INFO))) {
                    ...
                    if (CeSafeCopyMemory(&InfoLevel, pInBuf, sizeof(CE_VOLUME_INFO_LEVEL))) {
                        fRet = FSDMGR_GetVolumeInfo (pVolume, InfoLevel, pInfo, pBytesReturned);
                    }
