...
HRESULT GetDebugInfo(void* pModOrProc, BOOL fMod, ModuleSignature *pms)
{
    ...
    if (pe32l && pe32l->e32_unit[DEB].rva)
    {
...
DWORD FindTypeByNum(PROCESS *pVM, LPBYTE curr, DWORD id) 
{
    ...
    if (bytesread == sizeof (resroot_t))
    {
        ...
        while (!FoundEntry && root.numidents --)
        {
            resent_t res;
    ...
    ) 
{
    ...
    if (!(trva = FindTypeByNum(pVM, lpres, (DWORD)lpszType)) ||
       !(trva = FindTypeByNum(pVM, lpres+(trva&0x7fffffff),(DWORD)lpszName)) ||
       !(trva = FindFirst(pVM, lpres+(trva&0x7fffffff))) ||
       (trva & 0x80000000))
    {
        lpData = NULL;
        goto Exit;
