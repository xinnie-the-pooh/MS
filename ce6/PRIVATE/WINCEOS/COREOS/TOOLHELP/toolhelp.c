...
HANDLE WINAPI CreateToolhelp32Snapshot (DWORD dwFlags, DWORD th32ProcessID)
{
    ...
    if ((TH32CS_SNAPHEAPLIST & dwFlags)
    ...
    LPDWORD lpNumberOfBytesRead)
{
    ...
    if (!(hProc = OpenProcess(0,0,th32ProcessID))) {
...
BOOL IsValidHeapList (PTHSNAP pSnap, PTH32HEAPLIST php32)
{
    ...
    return (php32->dwTotalSize >= sizeof (TH32HEAPLIST))
        && (php32->dwTotalSize < pSnap->cbInuse)
        && (dwOfst < pSnap->cbInuse)
...
BOOL WINAPI Heap32First (HANDLE hSnapshot, LPHEAPENTRY32 lphe, DWORD th32ProcessID, DWORD th32HeapID)
{
    ...
    __try {
        ...
        if (hSnapshot
            ...
            && (php32 = FindHeap (pSnap, th32ProcessID, th32HeapID))) {
