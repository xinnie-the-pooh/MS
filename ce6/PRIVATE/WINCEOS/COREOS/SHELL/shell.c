...
const CMDCLASS v_commands[] =  {
    ...
    { 0, TEXT("rd"), DoHelp}, // DoResDump},
    { 0, TEXT("rf"), DoHelp}, // DoResFilter},
...
BOOL IsPfnInROM (DWORD dwPfn)
{
    return (dwPfn >= v_physInfo.dwPfnROMStart) && (dwPfn < v_physInfo.dwPfnROMEnd);
}
BOOL IsPfnInRAM (DWORD dwPfn)
{
    return (dwPfn >= v_physInfo.dwPfnRAMStart) && (dwPfn < v_physInfo.dwPfnRAMEnd);
}
...
WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR pCmdLine, int nShowCmd) {
    ...
    VERIFY (KernelLibIoControl ((HANDLE) KMOD_CORE, IOCTL_KLIB_GETPHYSMEMINFO,
...
BOOL DoHelp (PTSTR cmdline) {
    ...
    Puts(TEXT("kp <pid> [<pid2> <pid3> ...]: Kills process(es)\r\n"));
...
BOOL DoKillProc(PTSTR cmdline)
{
    ...
    //
    while (cmdline && NumberArg(&cmdline, &dwProcID))
    {
        ...
        if (dwProcID)
        {
            ...
            if (hProc) {        
                ...
                FmtPuts(TEXT("Unable to open process %d\r\n\r\n"), dwProcID);
            }
        }
        PrintSyntax = FALSE;
    }
    ...
    if (PrintSyntax)
    {
        Puts(TEXT("Syntax: kp <pid> [<pid2> <pid3> ...]\r\n\r\n"));
    }
...
BOOL DoDumpDwords (PTSTR cmdline) {
    ...
    if (cmdline == NULL || !NumberArg(&cmdline, &dwProcId)) {
        Puts(TEXT("Syntax: dd <process id> <addr> [<#dwords>]\r\n"));
        return TRUE;
    }

    if (!NumberArg(&cmdline, &ulAddr)) {
        Puts(TEXT("Syntax: dd <process id> <addr> [<#dwords>]\r\n"));
        return TRUE;
    }
...
IsNameMatch (LPCTSTR pszName, LPCTSTR pszText)
{
    ...
    //
    for (i = 0; i < NameLen; i++) {
        if (pszName[i] != szText[i]) {
            return FALSE;
        }
    }

    if ((WCHAR)'.' == szText[NameLen]) {
        return TRUE;
    }
...
BOOL DoGetInfo (PTSTR cmdline) {
    ...
    if (dwMask & SHELL_GETPROC) {
        if (INVALID_HANDLE_VALUE != hSnap) {
            ...
            if (Process32First(hSnap,&proc)) {
                cNumProc = 0;
                StringCbPrintf(rgbText, ccbrgbText, L"PROC: Name            hProcess: CurAKY :dwVMBase:CurZone\r\n");
                Puts(rgbText);
                if (dwMask & SHELL_GETTHRD) {
                    StringCbPrintf(rgbText, ccbrgbText, L"THRD: State :hCurThrd:hCurProc: CurAKY :Cp :Bp :Kernel Time  User Time\r\n");
                    Puts(rgbText);
                }
                do {
    ...
    if (dwMask & SHELL_GETSYSTEM) {
        ...
        if (GetVersionEx( &osv)) {
            ...
            StringCbPrintf(rgbText, ccbrgbText, L"Architecture CpuType    OSMajor    OSMinor    InstructionSet\r\n"
...
BOOL DoZoneOps (PTSTR cmdline) {
    ...
    if ((0 == cNumMod) || (0 == cNumProc)) {
        ...
        _tcsncpy(original, cmdline, MAX_PATH);
        original[MAX_PATH - 1] = 0;     // force null termination
    }
    ...
    case LOGACT_STOPFLUSH:
        {
            ...
            HANDLE hEvent = OpenEvent(EVENT_ALL_ACCESS, FALSE, CELOGFLUSH_QUITEVENT_NAME);
    ...
    if (!cmdline) {
        ...
        while (szToken && (szToken[0])) {
            if (!_tcscmp(szToken, TEXT("-buf"))) {
                ...
                if (!NumberArg(&szToken, &celog.dwBufSize)) {
                    DoLogHelp();
                    break;
                }
    ...
    )
{
    ...
    for (token = _tcstok(cmdline, TEXT(" \t")); token; token=_tcstok(NULL, TEXT(" \t"))) {
        if (!_tcsicmp(token, TEXT("timestamp"))) {
            ...
            if (token) {
                if (NumberArg (&token, &dwPri) && (dwPri < MAX_CE_PRIORITY_LEVELS)) {
        ...
        if (wAction == 1) {
            ...
            for (szToken = _tcstok(NULL, TEXT(" \t")); szToken; szToken = _tcstok(NULL, TEXT(" \t"))) {
                if (!_tcscmp(szToken, TEXT("-m"))) {
                    ...
                    szToken = _tcstok(NULL, TEXT(" \t"));
                    if (!NumberArg(&szToken, &dwProfilerInterval)) {
                        DoProfileHelp();
                        return TRUE;
                    }
