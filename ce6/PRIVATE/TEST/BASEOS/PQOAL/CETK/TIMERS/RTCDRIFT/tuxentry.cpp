...
BOOL                   
getTimeServerInfo (TCHAR *, DWORD, DWORD *);
...
BOOL
GetTimeStampValue (NtpTimeRecord *pStart, NtpTimeRecord *pEnd);

void 
CalcTimeElapsed (
    NtpTimeRecord *pStart, 
    NtpTimeRecord *pEnd, 
    LONGLONG *pNtpElapsed, 
    LONGLONG *pRtcElapsed);
                  ...
                  LPFUNCTION_TABLE_ENTRY lpFTE) 
{
    ...
    Info (_T(""));
    Info (_T("           Compares the RTC to an NTP server                "));
    Info (_T(""));
                            ...
                            LPFUNCTION_TABLE_ENTRY lpFTE) 
{
    ...
    /* server ip address */
    unsigned long ulServerIPHostByteOrder = 0; // never valid IP address
    ...
    LONGLONG ntpElapsed, rtcElapsed, lowerBound, fasterBound;
    
    GetTimeStampValue (&startRec, &endRec);
    CalcTimeElapsed (&startRec, &endRec, &ntpElapsed, &rtcElapsed);

    /* calculate bounds, fasterbound +2 because the round-down then should add 2 for compensation */
    lowerBound = (ntpElapsed / looseResolution - 1) * looseResolution;
    fasterBound = (ntpElapsed / looseResolution + 2) * looseResolution;
    rtcElapsed = (rtcElapsed / looseResolution) * looseResolution;
    
    Info (_T("Raw NTP: %I64d and raw RTC: %I64d."), ntpElapsed, rtcElapsed);

    if (rtcElapsed >= lowerBound && rtcElapsed <= fasterBound) 
    {
        Info (_T("No drift detected. NTP: %I64ds elapsed. RTC: %I64ds elapsed"), 
            ntpElapsed / SECOND_DIV_100NS, rtcElapsed / SECOND_DIV_100NS);
    }
    else
    {
        Error (_T("Detected drift. NTP: %I64d (*100ns) elapsed."), ntpElapsed);
        Error (_T("RTC should be between %I64d and %I64d inclusive. Instead, it's %I64d."), 
            lowerBound, fasterBound, rtcElapsed);
        goto cleanAndReturn;
    }
 ...
 */
BOOL
getTimeServerInfo (TCHAR * szTimeServerName, DWORD dwTimeServerNameMaxLength, 
                   DWORD * ipVersion)
{
    ...
    *ipVersion = 4;
    ...
    if (g_szDllCmdLine != NULL)
    {
        ...
        {
            _tcsncpy (szTimeServerName, szCmdString, dwTimeServerNameMaxLength);
...
getSleepTime (DWORD * pdwSleepTimeS)
{
    ...
    /* overflow? */
    if (ullSleepTimeS > (ULONGLONG) MAX_DWORD)
    {
    ...
    DWORD listMaxElements)
{
    ...
    pszToken = _tcstok(pszString, seps);
    while ( (pszToken = _tcstok(NULL, seps)) != NULL 
...
FindWorkableTimeServers (TCHAR *pszServerList[], DWORD serverNum)
{
        ...
        && (foundServerNum < NTP_ACTIVE_SERVERS); serverIndex++) 
    {
        ...
        if (NtpGetValidTimeStamp (serverIP, NULL)) 
        {
...
GetTimeStampBasedOnTimeServers (BOOL isStart)
{
    ...
    for (DWORD i = 0; i < g_ActiveServerNum; i++) 
    {
        if (g_ActiveServer[i].isValid) 
        {
            ...
            g_ActiveServer[i].isValid = NtpGetValidTimeStamp (g_ActiveServer[i].serverIpAddress, pTimeRec);
        }
        if (g_ActiveServer[i].isValid) {
            validCounts++;
        }
    ...
    LONGLONG *pRtcElapsed)
{
    ...
    *pNtpElapsed = ((tE1.QuadPart - tS1.QuadPart) + (tE2.QuadPart - tS2.QuadPart)) / 2;
    ...
    *pRtcElapsed = ((tE1.QuadPart - tS1.QuadPart) + (tE2.QuadPart - tS2.QuadPart)) / 2;
}
