...
SCardTuple::SCardTuple (UINT8 cisTplCode, UINT8 length, PCARD_DATA_PARMS pParms){
    	...
    	switch (uTupleCode){
        	...
        	case CISTPL_CFTABLE_ENTRY_CB:
          		nParsedItems = 0 ; // GLITCH
          		CardGetParsedTuple(hSocket, uTupleCode, 0, &nParsedItems);
          		parsedSize = nParsedItems * sizeof (PARSED_CBCFTABLE) ;
                                  ...
                                  PUINT pnItems)
{
    	...
    	switch (uDesiredTuple){
        	case CISTPL_CFTABLE_ENTRY_CB:
          		memcpy (pBuf, p->parsedBuf, nCount * sizeof(PARSED_CBCFTABLE)) ;
          		*pnItems = nCount ;
          		return CERR_SUCCESS ;
...
//------------------------------ HCard --------------------------------------
// BUGBUG: this class may not need to be existed at all
HCard::HCard () {;}
...
STATUS MatchedCard::CardGetTupleData (PCARD_DATA_PARMS p, UINT32 size){
    	...
    	if(size < sizeof(CARD_DATA_PARMS) || size > 0x10000)
    	    return CERR_BAD_ARGS;
...
{
      if(pBuf == NULL || actualBufSize > 0x10000)
        return CERR_BAD_ARGS;
    	...
    	if (uhDataLen == usDataLen){
        	switch (uDesiredTuple){
	            ...
	            case CISTPL_CFTABLE_ENTRY:
	              	nBufSize = usDataLen * sizeof (PARSED_CBCFTABLE) ;
	              	break ;
