...
TESTPROCAPI tst_IOCTL_DISK_FLUSH_CACHE(UINT uMsg,TPPARAM tpParam, 
...
// --------------------------------------------------------------------
{
	...
	DRIVER_TEST *pMyTest;
	pMyTest = new IOCTL_DISK_FLUSH_CACHE_TEST(g_hDisk,g_pKato,g_storageDeviceInfo.dwDeviceType);
	if(!pMyTest)
	{
		g_pKato->Log(LOG_FAIL,_T("FAILED to allocate a new IOCTL_DISK_FLUSH_CACHE_TEST class.\n"));
		return FALSE;
	}
...
BOOL IOCTL_DISK_FLUSH_CACHE_TEST::CustomFill(void)
{
	BOOL bRet = Fill();
	return bRet;
}

BOOL IOCTL_DISK_FLUSH_CACHE_TEST::LoadCustomParameters(void)
{
...
BOOL IOCTL_DISK_FLUSH_CACHE_TEST::RunIOCTL(void)
{
	try
	{
...
BOOL IOCTL_DISK_FLUSH_CACHE_TEST::ATAPIValidate(void)
{
	BOOL retVal = TRUE;
...
BOOL IOCTL_DISK_FLUSH_CACHE_TEST::Validate(void)
{
