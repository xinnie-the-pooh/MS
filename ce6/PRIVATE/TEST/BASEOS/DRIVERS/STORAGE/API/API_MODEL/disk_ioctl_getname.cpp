...
//
#include "main.h"
TESTPROCAPI tst_DISK_IOCTL_GETNAME(UINT uMsg,TPPARAM tpParam, 
...
// --------------------------------------------------------------------
{
	...
	DRIVER_TEST *pMyTest;
	pMyTest = new DISK_IOCTL_GETNAME_TEST(g_hDisk,g_pKato,g_storageDeviceInfo.dwDeviceType);
	if(!pMyTest)
	{
		g_pKato->Log(LOG_FAIL,_T("FAILED to allocate a new DISK_IOCTL_GETNAME_TEST class.\n"));
		return FALSE;
	}
...
BOOL DISK_IOCTL_GETNAME_TEST::CustomFill(void)
{
	BOOL bRet = Fill();
	return bRet;
}

BOOL DISK_IOCTL_GETNAME_TEST::LoadCustomParameters(void)
{
...
BOOL DISK_IOCTL_GETNAME_TEST::RunIOCTL(void)
{
	try
	{
...
BOOL DISK_IOCTL_GETNAME_TEST::ATAPIValidate(void)
{
	BOOL retVal = TRUE;
	...
	if(_tcscmp(lpOutBuffer,_T("NULL")) != 0 && _tcscmp(nOutBufferSize,_T("zero")) != 0)
	{
		...
		{
			expectedString = ATAPI_CF_DISK_NAME;
			...
			{
				expectedString = ATAPI_HD_DISK_NAME;
...
BOOL DISK_IOCTL_GETNAME_TEST::Validate(void)
{
