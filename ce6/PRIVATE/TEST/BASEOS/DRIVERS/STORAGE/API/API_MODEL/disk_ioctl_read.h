...
#ifndef _DISK_IOCTL_READ_H
#define _DISK_IOCTL_READ_H
#include "main.h"
...
typedef class _DISK_IOCTL_READ_TEST : public  DRIVER_TEST
{
...
public:
	_DISK_IOCTL_READ_TEST(void){m_pKato = NULL;m_hDevice = NULL;dwDeviceType = 0;inData.dwIoControlCode = DISK_IOCTL_READ;m_dataBuffer = NULL;}
	_DISK_IOCTL_READ_TEST(HANDLE Handle,CKato *pKato,DWORD deviceType){m_pKato = pKato;m_hDevice = Handle;dwDeviceType = deviceType;inData.dwIoControlCode = DISK_IOCTL_READ;m_dataBuffer = NULL;}
	~_DISK_IOCTL_READ_TEST(){if(m_dataBuffer)delete m_dataBuffer;m_dataBuffer = NULL;};
private:
...
}DISK_IOCTL_READ_TEST,*PDISK_IOCTL_READ_TEST;
#endif
