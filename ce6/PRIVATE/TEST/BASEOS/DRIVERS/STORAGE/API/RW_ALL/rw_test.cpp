...
WriteAllSectors (
    ...
    DWORD i = 0;
    DWORD dwThreadNumber, dwPercentComplete;
    HANDLE hDsk;
    ...
    if(g_szProfile[0])
        ...
        if ( dwThreadNumber == 1 ) 
            ...
            _stprintf( szTemp, TEXT("DSK%u:"), outIndex );
            }
        else 
            {        
            // Thread x will test DSKx:
            _stprintf( szTemp, TEXT("DSK%u:"), dwThreadNumber );
