...
// --------------------------------------------------------------------
{
    ...
    if(cSectorsPerReq > MaxSectorPerOp(&g_diskInfo)) 
    {
        g_pKato->Log(LOG_SKIP, _T("SKIP: %u sectors is larger than maximum allowed (%u)"),
            cSectorsPerReq, MaxSectorPerOp(&g_diskInfo));
        return TPR_SKIP;
    }
...
// --------------------------------------------------------------------
{
    ...
    if(ReadWriteDisk(g_hDisk, IOCTL_DISK_WRITE, &g_diskInfo, 0, 0, NULL))
    {
    ...
    {
        g_pKato->Log(LOG_FAIL, _T("FAIL: TestReadWriteInvalidBuffer(WRITE) with empty buffer - failed with wrong error code %u"), GetLastError());
        goto Fail;
    }

    // Invalid Case # 2 - Read with NULL buffer
    if(ReadWriteDisk(g_hDisk, IOCTL_DISK_READ, &g_diskInfo, 0, 0, NULL))
    {
    ...
    {
        g_pKato->Log(LOG_FAIL, _T("FAIL: TestReadWriteInvalidBuffer(READ) with empty buffer - failed with wrong error code %u"), GetLastError());
        goto Fail;
    }	
    ...
    {
	    if(ReadWriteDisk(g_hDisk, IOCTL_DISK_WRITE, &g_diskInfo, 0, 10, (PBYTE)addr))
	    {
	    ...
	    {
	        g_pKato->Log(LOG_FAIL, _T("FAIL: TestReadWriteInvalidBuffer(WRITE) with buffer addr 0x%lx - failed with wrong error code %u"), addr, GetLastError());
		    goto Fail;
	    }
		
	    // Invalid Case # 4 - Read with invalid buffer (buffer address reside in the Kernel VM (0xc000000 - 0xd000000) )	
	    if(ReadWriteDisk(g_hDisk, IOCTL_DISK_READ, &g_diskInfo, 0, 10, (PBYTE)0xc1234567))
	    {
	    ...
	    {
	        g_pKato->Log(LOG_FAIL, _T("FAIL: TestReadWriteInvalidBuffer(READ) with buffer addr 0x%lx - failed with wrong error code %u"), addr, GetLastError());
		 goto Fail;
	    }
    }
...
// --------------------------------------------------------------------
{
    ...
    {
        g_pKato->Log(LOG_FAIL, _T("FAILFAIL: TestReadWriteInvalidMaxSG(WRITE) exceed max # of SG_BUF - failed with wrong error code %u"), GetLastError());
	 goto Fail;
    }		
...
// --------------------------------------------------------------------
{
    ...
    {
        g_pKato->Log(LOG_FAIL, _T("FAILFAIL: TestReadWriteInvalidSG(WRITE) contains an empty SG_BUF - failed with wrong error code %u"), GetLastError());
	 goto Fail;
    }		
