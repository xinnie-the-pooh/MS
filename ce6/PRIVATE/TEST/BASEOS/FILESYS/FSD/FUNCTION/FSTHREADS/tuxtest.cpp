...
void ParseCommandLine(LPTSTR szCmdLine) ;
extern int 			g_iDevice;
extern WCHAR        g_szProfile[];
...
SHELLPROCAPI ShellProc(UINT uMsg, SPPARAM spParam)
{
    ...
    case SPM_SHELL_INFO:
        ...
        ParseCommandLine((LPTSTR)g_pShellInfo->szDllCmdLine);
        break;
...
void ParseCommandLine(LPTSTR szCmdLine) 
{
    ...
    if(!cmdLine.GetOptString(L"profile", g_szProfile, PROFILENAMESIZE)) {
        g_szProfile[0] = L'\0';
    }

    if(!cmdLine.GetOptVal(L"device", (LPDWORD)&g_iDevice)) {
        WCHAR *pszValue;
        if ((pszValue = _tcsstr(szCmdLine, _T("device="))))
             g_iDevice = _ttoi(pszValue + _tcslen(_T("device=")));
    }
}
