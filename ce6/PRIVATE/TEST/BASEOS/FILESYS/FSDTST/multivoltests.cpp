...
TESTPROCAPI Tst_Depth32(UINT uMsg, TPPARAM tpParam, LPFUNCTION_TABLE_ENTRY lpFTE )
{
	...
	_stprintf(szDirName, _T("%s\\%s"), szVolumeName, TEST_DIR_NAME);
	...
	_stprintf(szSubDirName, _T("%s\\%s02"), szDirName, TEST_DIR_NAME);
	...
	_stprintf(szFileName, L"%s\\%s", szSubDirName, TEST_FILE_NAME);
	...
	_stprintf(szRelocatedDirName, _T("%s\\%s02"), szVolumeName, TEST_DIR_NAME);
	
	// Write out new string containg the relocated file name
	_stprintf(szFileName, _T("%s\\%s"), szRelocatedDirName, TEST_FILE_NAME);
