...
INT StencilMaskTest(LPDIRECT3DMOBILEDEVICE pDevice, HWND hWnd, UINT uiTestPermutation)
{
	...
	{
		DebugOut(DO_ERROR, L"Device doesn't support the parameters for this test. Skipping.");
		Result = TPR_SKIP;
		goto cleanup;
	}
	...
	//
	if (FAILED(hr = pDevice->Clear( 0L, NULL, D3DMCLEAR_STENCIL | D3DMCLEAR_TARGET | D3DMCLEAR_ZBUFFER, D3DMCOLOR_XRGB(255,0,0) , 1.0f, StencilMaskTests[uiTestPermutation].InitialStencilValue )))
	{
