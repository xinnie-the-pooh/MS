...
static TCHAR g_tszRecordingPath[MAX_PATH];
...
void TestSuite_Init()
{
    const TCHAR * pszFnName = TEXT("TestSuite_Init");
    DWORD dwSize = 300000;        // FileSize
    DWORD dwError = 0;
    int i = 0;

    if(FAILED(CoInitialize(NULL)))
    {
        LogText(__LINE__, pszFnName, TEXT("Com initialize failed\n"));
    }
    else
    {
        LogText(__LINE__, pszFnName, TEXT("Com initialize Succeed\n"));
    }

    dwSize = MAX_MY_PERMANENT_FILE_SIZE;
    dwError = Helper_DvrRegSetValueDWORD(MAX_PERMANENT_FILE_SIZE_KEY, &dwSize);
    dwSize = MAX_MY_TEMPORARY_FILE_SIZE;
    dwError = Helper_DvrRegSetValueDWORD(MAX_TEMPORARY_FILE_SIZE_KEY, &dwSize);
    //read my size
    dwError = Helper_DvrRegQueryValueDWORD(MAX_TEMPORARY_FILE_SIZE_KEY, &dwSize);
    LogText(__LINE__, pszFnName, TEXT("my Temp size is %d"), dwSize);
    dwError = Helper_DvrRegQueryValueDWORD(MAX_PERMANENT_FILE_SIZE_KEY, &dwSize);
    LogText(__LINE__, pszFnName, TEXT("my Perm size is %d"), dwSize);

    // now lets output the current command line configuration information for the user's reference
    {
        CSinkFilterGraph SinkGraph;
        CSourceFilterGraph SourceGraph;

        SinkGraph.SetCommandLine(g_pShellInfo->szDllCmdLine);
        SourceGraph.SetCommandLine(g_pShellInfo->szDllCmdLine);

        LogText(TEXT("Sink Graph configuration:"));
        SinkGraph.OutputCurrentCommandLineConfiguration();
        LogText(TEXT("Source Graph configuration:"));
        SourceGraph.OutputCurrentCommandLineConfiguration();
    }

}

void TestSuite_Cleanup()
{
    const TCHAR * pszFnName = TEXT("TestSuite_Cleanup");
    DWORD dwSize = 0;
    DWORD dwError = 0;

    CoUninitialize();

    LogText(__LINE__, pszFnName, TEXT("Com Uninitialized\n"));

    // restore filesize
    //try set old size
    dwSize = MAX_PERMANENT_FILE_SIZE;
    dwError = Helper_DvrRegSetValueDWORD(MAX_PERMANENT_FILE_SIZE_KEY, &dwSize);
    dwSize = MAX_TEMPORARY_FILE_SIZE;
    dwError = Helper_DvrRegSetValueDWORD(MAX_TEMPORARY_FILE_SIZE_KEY, &dwSize);
    // read old size
    dwError = Helper_DvrRegQueryValueDWORD(MAX_TEMPORARY_FILE_SIZE_KEY, &dwSize);
    LogText(__LINE__, pszFnName, TEXT("Old Temp size is %d"), dwSize);
    dwError = Helper_DvrRegQueryValueDWORD(MAX_PERMANENT_FILE_SIZE_KEY, &dwSize);
    LogText(__LINE__, pszFnName, TEXT("Old Perm size is %d"), dwSize);
}
...
SHELLPROCAPI ShellProc(UINT uMsg, SPPARAM spParam)
{
    ...
    case SPM_SHELL_INFO:
        ...
        if(FAILED(CoInitialize(NULL)))
        {
            Debug(TEXT("Failed to initialize COM"));
            return SPR_FAIL;
        }

        // create a source and sink graph, let them parse the command line and output whether or not it's valid.
        {
            CSinkFilterGraph SinkGraph;
            CSourceFilterGraph SourceGraph;

            if(FAILED(SinkGraph.RegisterTestDLLs()) || FAILED(SourceGraph.RegisterTestDLLs()))
            {
                Debug(TEXT("Failed to register necessary test Dll's, the tests cannot run."));
                CoUninitialize();
                return SPR_FAIL;
            }

            BOOL bCmdLine1 = SinkGraph.SetCommandLine(g_pShellInfo->szDllCmdLine);
            BOOL bCmdLine2 = SourceGraph.SetCommandLine(g_pShellInfo->szDllCmdLine);
            if(!bCmdLine1 || !bCmdLine2)
            {
                Debug(TEXT("Parsing the command line failed (expected if options requested)."));
                CoUninitialize();
                return SPR_FAIL;
            }

            // Retrieve a copy of the current recording path for later cleanup.
            SinkGraph.GetDefaultRecordingPath(g_tszRecordingPath);
        }
    ...
    case SPM_END_TEST:
        ...
        g_pKato->EndLevel(
            ...
            // The test completed, clean up after it
            Helper_CleanupRecordPath(g_tszRecordingPath);
        break;
    ...
    case SPM_EXCEPTION:
        ...
        g_pKato->Log(0, TEXT("Exception occurred!"));
        break;
