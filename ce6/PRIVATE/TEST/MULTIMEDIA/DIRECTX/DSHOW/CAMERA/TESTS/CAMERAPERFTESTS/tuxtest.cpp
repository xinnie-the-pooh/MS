...
// the global graph pointer used for initialization.
extern CAPTUREFRAMEWORK g_CaptureGraph;
extern int g_nAudioDeviceCount;
extern TESTVECTOR g_VectorData;
extern CAMERACSV g_CSVLogger;

TCHAR g_szINIPath[MAX_PATH] = TEXT("CapConfig.ini");
TCHAR g_szResultsPath[MAX_PATH] = TEXT("CapResults.csv");
...
{
    class CClParse cCLPparser(szDLLCmdLine);
    TCHAR tszTemp[MAX_PATH];
    BOOL bResult = TRUE;

    // first, let the capture framework parse the command line to determine whether or not it's valid.
    bResult = g_CaptureGraph.ParseCommandLine(szDLLCmdLine);

    // now, regardless of passing or failing of the capture framework, see if the user requested the command
    // line options
    if(cCLPparser.GetOpt(TEXT("?")))
    {
        Log(TEXT("usage:  s tux -o -d CameraPerfTests -c \"DLL parameters\""));
        Log(TEXT("Tux parameters:  please refer to s tux -?"));
        Log(TEXT("DLL parameters: [-d] [-l] [-p]"));
        Log(TEXT("\t-?\t this help message"));                      
        Log(TEXT("\t-p\t full path of where the results will be, for example '\\storage card\\capresults.csv'."));
        Log(TEXT("\t-c\t full path of where the ini file is, for example '\\storage card\\capconfig.ini'."));
        bResult = FALSE;
    }

    // As long as the user didn't request the command line options, and the framework's parsing of the command
    // line succeeded, then complete the command line parsing.  Othwerwise, return failure.
    if(bResult)
    {
        if(cCLPparser.GetOptString(TEXT("p"), tszTemp, MAX_PATH))
            _tcscpy(g_szResultsPath, tszTemp);

        if(cCLPparser.GetOptString(TEXT("c"), tszTemp, MAX_PATH))
            _tcscpy(g_szINIPath, tszTemp);
    }
...
SHELLPROCAPI ShellProc(UINT uMsg, SPPARAM spParam)
{
    ...
    case SPM_UNLOAD_DLL:
        ...
        g_CSVLogger.Cleanup();
        g_VectorData.Cleanup();
        g_CaptureGraph.Cleanup();
    ...
    case SPM_SHELL_INFO:
        ...
        if(!ProcessCommandLine(g_pShellInfo->szDllCmdLine))
        {
        ...
        // if the process command line fails, then that means the user requested the command
        // line options, so exit the dll.
        if(FAILED(g_CaptureGraph.InitCameraDriver()))
        {
            Debug(TEXT("Unable to intialize camera driver, no driver available."));
            return SPR_FAIL;
        }

        // get the ini file name from the command line
        if(FAILED(g_VectorData.Init(g_szINIPath)))
        {
            FAIL(TEXT("Failed to initialize the vector data.  Is the Vector data file available? Run the test with -c \"/?\" for the test command line options."));
            return SPR_FAIL;
        }

        if(!g_CSVLogger.Init(g_szResultsPath[0]?g_szResultsPath:NULL, &g_CaptureGraph, &g_VectorData))
        {
            FAIL(TEXT("Failed to initialize the logger. Is the path to the csv file correct? Run the test with -c\"/?\" for the test command line options."));
            return SPR_FAIL;
        }

        // retrieve the number of audio devices available
        g_nAudioDeviceCount = waveInGetNumDevs();
    ...
    case SPM_BEGIN_GROUP:
        ...
        g_CaptureGraph.OutputRuntimeParameters();

        if(g_nAudioDeviceCount > 0)
        {
            g_pKato->Log(LOG_DETAIL, (TEXT("    Audio is available.")));
        }
