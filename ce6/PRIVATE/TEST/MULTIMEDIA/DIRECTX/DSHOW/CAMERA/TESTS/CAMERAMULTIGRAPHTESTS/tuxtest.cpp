...
extern PLAYBACKFRAMEWORK g_PlaybackGraph;
...
SHELLPROCAPI ShellProc(UINT uMsg, SPPARAM spParam)
{
    ...
    case SPM_SHELL_INFO:
        ...
        if(!g_CaptureGraph.ParseCommandLine(g_pShellInfo->szDllCmdLine))
        {
            // execute the playback command parser, in case the request was for command line options.
            g_PlaybackGraph.ParseCommandLine(g_pShellInfo->szDllCmdLine);
            Debug(TEXT("invalid command line, failing the test suite so it won't continue."));
            return SPR_FAIL;
        }

        // Now that the capture command line was successfully parsed, check the playback command line
        if(!g_PlaybackGraph.ParseCommandLine(g_pShellInfo->szDllCmdLine))
        {
            Debug(TEXT("invalid command line, failing the test suite so it won't continue."));
            return SPR_FAIL;
        }
    ...
    case SPM_BEGIN_GROUP:
        ...
        g_PlaybackGraph.OutputRuntimeParameters();
    ...
    case SPM_BEGIN_TEST:
        ...
        // send the test the begin/end test messages for initialization. If the test
        // doesn't know how to handle it, it returns not handled.
        (pBT->lpFTE->lpTestProc)(uMsg, NULL, pBT->lpFTE);
        break;
    ...
    case SPM_END_TEST:
        ...
        // send the test the begin/end test messages for initialization. If the test
        // doesn't know how to handle it, it returns not handled.
        (pET->lpFTE->lpTestProc)(uMsg, NULL, pET->lpFTE);
