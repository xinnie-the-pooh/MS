...
HRESULT NtEnumAdapters(PtsNtEnumAda psNtEnumAd, DWORD dwMaxNosOfAda)
{
	...
	memset(psNtEnumAd,0,sizeof(tsNtEnumAda));

    hr = HrGetINetCfg( FALSE,NULL,&pnc,&lpszApp );
    if ( hr == S_OK ) {
        // Get Component Enumerator Interface.
        hr = HrGetComponentEnum( pnc,pguidNetClass[3],&pencc );
        if ( hr == S_OK ) {
            hr = HrGetFirstComponent( pencc, &pncc );
			DWORD& i = psNtEnumAd->dwNosOfAda;
			i = 0;
            while( (hr==S_OK) && (i<dwMaxNosOfAda) ) {
				LPWSTR                  lpszName;
				hr = pncc->GetDisplayName( &lpszName );
                if ( hr == S_OK ) {
					_tcscpy(psNtEnumAd->sNtAdaArray[i].szwDisplayName,lpszName);
                    CoTaskMemFree( lpszName );
                }
				hr = pncc->GetBindName( &lpszName );
                if ( hr == S_OK ) {
					_tcscpy(psNtEnumAd->sNtAdaArray[i].szwBindName,lpszName);
                    CoTaskMemFree( lpszName );
                }
				hr = pncc->GetHelpText( &lpszName );
                if ( hr == S_OK ) {
					_tcscpy(psNtEnumAd->sNtAdaArray[i].szwHelpText,lpszName);
                    CoTaskMemFree( lpszName );
                }
				hr = pncc->GetId( &lpszName );
                if ( hr == S_OK ) {
					_tcscpy(psNtEnumAd->sNtAdaArray[i].szwId,lpszName);
                    CoTaskMemFree( lpszName );
                }
