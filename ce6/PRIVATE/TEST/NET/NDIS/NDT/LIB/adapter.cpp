...
cleanUp:
   ...
   if (pulSend != NULL) *pulSend = ulSend;
   if (pulReceive != NULL) *pulReceive = ulReceive;
   return hr;
}
...
HRESULT CAdapter::Status(ULONG ulStatus, CRequest*& pRequest)
{
   HRESULT hr = S_OK;
   HRESULT hrRequest = S_OK;
   ...
   // Marshal input parameters
   hr = pRequest->MarshalInpParameters(NDT_MARSHAL_INP_STATUS_START, m_hBinding, ulStatus);
   if (FAILED(hr)) goto cleanUp;

   // Execute request (async)
   hr = pRequest->Execute(0, NDT_REQUEST_STATUS_START);
   if (FAILED(hr)) goto cleanUp;
...
HRESULT CAdapter::StatusWait(CRequest* pRequest, DWORD dwTimeout)
{
   HRESULT hr = S_OK;
   HRESULT hrRequest = S_OK;
...
HRESULT CAdapter::ClearCounters()
{
   HRESULT hr = S_OK;
   HRESULT hrRequest = S_OK;
   CRequest* pRequest = NULL;
   ...
   // Marshal input parameters
   hr = pRequest->MarshalInpParameters(NDT_MARSHAL_INP_CLEAR_COUNTERS, m_hBinding);
   if (FAILED(hr)) goto cleanUp;

   // Execute request
   hr = pRequest->Execute(NDT_SYNC_REQUEST_TIMEOUT, NDT_REQUEST_CLEAR_COUNTERS);
   if (FAILED(hr)) goto cleanUp;

   // Unmarshal output parameters
   hr = pRequest->UnmarshalOutParameters(NDT_MARSHAL_OUT_CLEAR_COUNTERS, &hrRequest);
   if (FAILED(hr)) goto cleanUp;
