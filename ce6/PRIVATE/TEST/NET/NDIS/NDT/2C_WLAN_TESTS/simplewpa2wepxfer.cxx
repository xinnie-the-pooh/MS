...
TEST_FUNCTION(SimpleWPA2WEPXfer)
{
   ...
   for (ix= 0; ix<dwSsidCount; ix++)
   {
      ...
      if (NOT_SUCCEEDED(hr))
      {
         ...
         NDTLogMsg(_T("Succesfully associated support with %s"),szSsidText);
      ...
      if (NOT_SUCCEEDED(hr))
      {
         ...
         break;
          ...
          NDTLogMsg(_T("Succesfully associated test with %s"),szSsidText);
      ...
      if (ulPacketsReceived < ulMinDirectedPass)
      {
         ...
         FlagError(ErrorSevere,&rc);
   	   ...
   	   NDTLogMsg(_T("Directed Packets ulPacketsToSend %d,ulPacketsSent %d, ulPacketsReceived %d"),ulPacketsToSend , ulPacketsSent, ulPacketsReceived);
