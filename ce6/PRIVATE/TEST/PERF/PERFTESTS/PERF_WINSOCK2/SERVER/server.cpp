...
LogLevel g_enLogLevel = DEBUG_MSG;
#else
LogLevel g_enLogLevel = REQUIRED_MSG;
#endif
...
unsigned long __stdcall ServerMainThread(void* pParams)
{
    ...
    SOCKET sockAccept = SOCKET_ERROR;
    unsigned long nTemp;
    ...
    LogInitialize(TEXT("PERF_WINSOCKD2"),g_enLogLevel);
...
{
	//Simple hack to serve option z: Verbosity
	for (DWORD i=0;i<dwArgc;i++)
	{
		if (_tcsicmp(lpszArgv[i],_T("-z"))==0)
		{
			g_enLogLevel = LogLevel((UINT)_ttoi(lpszArgv[i+1]));

			if (g_enLogLevel > REQUIRED_MSG) 
				g_enLogLevel = REQUIRED_MSG;
			else if (g_enLogLevel < DEBUG_MSG)
				g_enLogLevel = DEBUG_MSG;
