...
// --------------------------------------------------------------------
{
    ...
    for (INT i=0; i<g_nTestBufferCount; i+=1)
    {
        ...
        {
            Log(ERROR_MSG, TEXT("CTestTCPSendThroughput test failed"));
            nResult = TPR_FAIL;
            goto Cleanup;
        }
    }
...
// --------------------------------------------------------------------
{
    ...
    for (INT nBufferIndex = 0; nBufferIndex < g_nTestBufferCount; nBufferIndex++)
    {
        DWORD dwBufSize = g_aTestBufferSizes[nBufferIndex];
		DWORD dwNumBufs = 0;
...
// --------------------------------------------------------------------
{
    ...
    for (INT nBufferIndex = 0; nBufferIndex < g_nTestBufferCountUdp; nBufferIndex++)
    {
        DWORD dwBufSize = g_aTestBufferSizesUdp[nBufferIndex];
        DWORD dwNumBufs = 0;
		if (g_fUserSelectedNumOfBuffers)
		{
			dwNumBufs = g_dwNumberOfBuffers;
