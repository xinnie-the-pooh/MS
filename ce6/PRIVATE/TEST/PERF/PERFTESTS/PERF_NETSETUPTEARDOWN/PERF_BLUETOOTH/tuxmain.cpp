...
BT_ADDR	g_BTServerAddr;
unsigned int g_BTServerChannel;
bool g_BTServer;
int g_nTotalTransferSize;
int g_nBTMtu;
int g_nIterations;
...
#define MAX_COMMAND_LINE_ELEM	32
			...
			// Initialize Global Variables - all globals need to be initialized
			// to proper values through options.

			g_BTServerAddr = SET_NAP_SAP (0,0);
			g_BTServerChannel = 0;
			g_BTServer = FALSE;
			g_nTotalTransferSize = 0;
			g_nBTMtu = 0;
			g_nIterations = 1;
			...
			if (g_pShellInfo->szDllCmdLine && *g_pShellInfo->szDllCmdLine)
			{
				...
				if((WasOption(argc, argv, L"?") >= 0) || (WasOption(argc, argv, L"help") >= 0))
				{
					ShowUsage();
					return SPR_FAIL;
				}
				...
				// Bluetooth options: 
				// -s <server address>, -b <total data transfer size in bytes>, -p <server channel number>
				// -m <Bluetooth TDI MTU size>
				// Client: -s, -b and -p are mandatory, -m is excluded/ignored
				// Server: -b and -p are mandatory, -s is excluded

				TCHAR szOptionArray [][5] = {L"s", L"b", L"p", L"m", L"i"};
				...
				for (i = 0; i < numberOfOptions; i++)
				{
					...
					if (woRetVal < -1)
					{
						...
						g_pKato->Log(LOG_FAIL, TEXT("Error parsing command line for option -%s"), szOptionArray[i]);
						return SPR_FAIL;
					}
					else if (woRetVal == -1)
					{
						...
						if (i == 0) // -s option
						{
							// if -s is missing, this is server
							g_BTServer = TRUE;
						}
						else
							if (i == 3) // -m option
						...
						{
							// if something else is missing, it is an error
							g_pKato->Log(LOG_FAIL, TEXT("Option -%s is missing, -s, -b and -p are mandatory"), szOptionArray[i]);
							return SPR_FAIL;
						}
					}
					else
					{
						if (i == 0) // -s option
						{
							...
							// Convert the address from string to number
							if (!GetBA (lpszGetOptStr, &g_BTServerAddr))
							{
								g_pKato->Log(LOG_FAIL, TEXT("Error parsing BT address of server"));
								return SPR_FAIL;
							}
						}
						...
						if (i == 1) // -b option
						{
							GetOptionAsInt(argc, argv, szOptionArray[i], (int*)&g_nTotalTransferSize);
						}

						if (i == 2) // -p option
						{
							GetOptionAsInt(argc, argv, szOptionArray[i], (int*)&g_BTServerChannel);
						}

						if (i == 3) // -m option
						{
							GetOptionAsInt(argc, argv, szOptionArray[i], (int*)&g_nBTMtu);
						}

						if (i == 4) // -i option
						{
							GetOptionAsInt(argc, argv, szOptionArray[i], (int*)&g_nIterations);
							g_pKato->Log(LOG_COMMENT, TEXT("Number of iterations: %d"), g_nIterations);
						}
					}
				}

				if (!Perf_SetTestName (MODULE_NAME))
				{
					g_pKato->Log(LOG_FAIL, TEXT("Error setting test name"));
					return SPR_FAIL;
				}
			}
      ...
      //------------------------------------------------------------------------
        case SPM_BEGIN_GROUP:
		{
            ...
            g_pKato->BeginLevel(0, _T("BEGIN GROUP: PERF_BLUETOOTH.DLL"));
      ...
      //------------------------------------------------------------------------
        case SPM_END_GROUP:
		{
            ...
            g_pKato->EndLevel(_T("END GROUP: PERF_BLUETOOTH.DLL"));
...
VOID ShowUsage()
{
	Debug(TEXT("PERF_BLUETOOTH: Usage: tux -o -d perf_bluetooth -c\"-s <serverAddress> -b <dataSize> -p <serverChannelNumber>\"\n"));
	Debug(TEXT("PERF_BLUETOOTH: Client: all options are mandatory\n"));
	Debug(TEXT("PERF_BLUETOOTH: Server: -s is excluded, all other options are mandatory and should match client options\n"));
	Debug(TEXT("PERF_BLUETOOTH: -s <serverAddress>       : Bluetooth address of server in 12-digit hex format (with no 0x at front)\n"));
	Debug(TEXT("PERF_BLUETOOTH: -b <dataSize>            : Number of bytes to transfer for data transfer stress test\n"));
	Debug(TEXT("PERF_BLUETOOTH: -p <serverChannelNumber> : RFCOMM channel at which server is listening\n"));
	Debug(TEXT("PERF_BLUETOOTH: -i <iterations> : number of iterations (default 1)\n"));
}
