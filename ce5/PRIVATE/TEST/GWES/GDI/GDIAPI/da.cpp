...
TestSetBkModeRandom(void)
{
    ...
    BYTE mode = rand() % 0xFF;
    info(ECHO, TEXT("TestSetBkModeRandom: mode = %d"), mode);
...
CheckGetSetStretchBltModes()
{
    ...
    nModeSet = nModesKnown[rand() % countof(nModesKnown)];
    ...
    for(int i=0; i < 512; i++)
    {
        ...
        nNewMode = nModesKnown[rand() % countof(nModesKnown)];
