...
TplTest::Init(){
	...
	if(m_pTplContents == NULL){
        	g_pKato->Log(LOG_DETAIL,TEXT("FAILED --> out fo memory  -- Thread%d, Socket %d\r\n"), 
	...
	if(ReadAllTuples(m_pTplContents, NULL) == FALSE){
        	g_pKato->Log(LOG_DETAIL,TEXT("FAILED --> Failed to read out all tuples on this card  -- Thread%d, Socket %d\r\n"), 
        							m_dwThreadID, m_uLocalSock);
		return FALSE;
	}
...
BOOL TplTest::GetOneTypeTuples(PTPLCONTENT pTpls, INT loc){
	...
	do{
		//get data from this tuple
	    	...
	    	if (size < sizeof(CARD_TUPLE_PARMS)) 
	    		return FALSE ;
...
VOID TplTest::DumpCurrentErrors (){
	...
	m_pMatchedCard->getMismatchedParams (&w, &x, &y) ;
    	...
    	switch (w){
        	case 0: // STATUS
          		g_pKato->Log(LOG_DETAIL,TEXT("FAILED --> Soft status: %s  --  Hard status: %s -- Thread%d, Sock %u Func %u\r\n"),
        	...
        	case 1: // Tuple code
          		...
          		g_pKato->Log(LOG_DETAIL,TEXT("FAILED --> Soft card code: %s  --  Hard card code: %s -- Thread%d, Sock %u Func %u\r\n"),
        	...
        	case 2: // Tuple Link
          		g_pKato->Log(LOG_DETAIL,TEXT("FAILED  --> Soft tuple link: %ld  --  Hard tuple link: %ld -- Thread%d, Sock %u Func %u\r\n"),
        	...
        	case 3: // Data Length
          		g_pKato->Log(LOG_DETAIL,TEXT("FAILED --> Soft data length: %ld  --  Hard data length: %ld -- Thread%d, Sock %u Func %u\r\n"),
              							x, y, m_dwThreadID, m_uLocalSock, m_uLocalFunc) ;
          		updateError (TST_SEV2) ;
          		SetResult(FALSE);
          		break ; 
        	...
        	case 4: // Data mismatch
          		g_pKato->Log(LOG_DETAIL,TEXT("FAILED --> Tuple data does not match -- Both lengths: %ld -- Thread%d, Sock %u Func %u\r\n"),
        	...
        	case 5: // Extra data bytes set.
          		g_pKato->Log(LOG_DETAIL,TEXT("FAILED --> The tuple has %ld extra bytes -- Thread%d, Sock %u Func %u\r\n"),
        	...
        	default: // No errors
          		g_pKato->Log(LOG_DETAIL,TEXT("PASSED -- Thread%d, Sock %u Func %u\r\n"),
              							m_dwThreadID, m_uLocalSock, m_uLocalFunc) ;
          		break ; 
      }
...
VOID TplTest::CardGetFirstTuple (){
	...
	DEBUGMSG(ZONE_FUNCTION, (TEXT("+ TplTest::CardGetFirstTuple() enterted\r\n")));
    	g_pKato->Log(LOG_DETAIL,TEXT("--> CardGetFirstTuple: %s  --  Desired: %s -- Thread%d Sock %u Func %u\r\n"),
