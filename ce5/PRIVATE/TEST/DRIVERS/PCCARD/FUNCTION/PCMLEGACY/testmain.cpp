...
PFN_GetSocketStatus	pfnGetSocketStatus;
PFN_GetSocketIndex	pfnGetSocketIndex;
PFN_EnumCard		pfnEnumCard;
PFN_EnumSocket		pfnEnumSocket;
...
TESTPROCAPI TestDispatchEntry(UINT uMsg, TPPARAM tpParam, LPFUNCTION_TABLE_ENTRY lpFTE)
{
	...
	if((USHORT)dwTestCat != uTestCategory) {
		...
		if(FALSE == TestSetup(uTestCategory)){
			...
			if(pfnEnumCard(&dwCardsNum, g_CardDescs, &dwCardsAvail) != CERR_SUCCESS){
...
TestSetup(USHORT uTestCat){
	...
	if(pfnEnumSocket(&dwSockets, g_SocketDescs, &dwSocketsAvail) != CERR_SUCCESS){
	...
	for(i = 0; i < (int)dwSocketsAvail; i++){//set up 
		...
		//get socket status
		dwSocketStatus = 0;
		if (CERR_SUCCESS == pfnGetSocketStatus(SocketInfo.dwSocketIndex, &dwSocketStatus)){
