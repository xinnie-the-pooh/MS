...
{    
    Debug(TEXT(""));
    Debug(TEXT("Usage: tux -o -d blkioctl -c\"/disk <disk> /nostrict\""));
    Debug(TEXT("       /disk <disk>        : name of the disk to test (e.g. DSK1:); default = first detected data cd"));
    Debug(TEXT("       /nostrict           : do not use strict API requirements"));
    Debug(TEXT(""));
...
{
    static  TCHAR   szHeader[] = TEXT("BLKIOCTL: ");
    TCHAR   szBuffer[1024];
...
//
// Parameters:
//  lpShellInfo     shell information from TUX
//
// Return value:
//  none

void Initialize(LPSPS_SHELL_INFO lpShellInfo)
{
    // Store a pointer to our shell info for later use.
    g_pShellInfo = lpShellInfo;
    CClParse cmdLine(lpShellInfo->szDllCmdLine);

    if(!cmdLine.GetOptString(TEXT("disk"), g_szDisk, MAX_PATH))
    {
        VERIFY(SUCCEEDED(StringCchCopy(g_szDisk, MAX_PATH, TEXT(""))));
    }

    g_fFailOnWarn = !cmdLine.GetOpt(TEXT("nostrict"));

    g_hDisk = GetDiskHandle(g_szDisk, &g_diskInfo);
}

////////////////////////////////////////////////////////////////////////////////
// Shutdown
//  Cleans up global data
//
// Parameters:
//  none
//
...
SHELLPROCAPI ShellProc(UINT uMsg, SPPARAM spParam)
{
    ...
    case SPM_SHELL_INFO:
        ...
        Initialize((LPSPS_SHELL_INFO)spParam);
    ...
    case SPM_BEGIN_GROUP:
        ...
        g_pKato->BeginLevel(0, TEXT("BEGIN GROUP: BLKIOCTL.DLL"));
        break;
    ...
    case SPM_END_GROUP:
        ...
        g_pKato->EndLevel(TEXT("END GROUP: BLKIOCTL.DLL"));
        break;
