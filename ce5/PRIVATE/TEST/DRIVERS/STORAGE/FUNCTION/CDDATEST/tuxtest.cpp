...
{
    static  TCHAR   szHeader[] = TEXT("CDDATEST: ");
    TCHAR   szBuffer[1024];
...
{
    Debug(TEXT(""));
    Debug(TEXT("usage: tux -o -d cddatest -c\"/disk <disk> /output <dir> /track <track no.>"));
    Debug(TEXT("       /disk <disk>       : name of the disk to test (e.g. DSK1:); default = first detected CDDA cd"));
    Debug(TEXT("       /profile <profile> : limit to devices of the specified storage profile; default = all profiles"));
    Debug(TEXT("       /output <dir>      : output directory for generated .wav files; default = \\release"));
    Debug(TEXT("       /track <track no.> : track number to copy to .wav file; default = 1"));
    Debug(TEXT(""));
}

////////////////////////////////////////////////////////////////////////////////
// Initialize
//  Process the command line and perform any initialization.
//
// Parameters:
//  None.
//
...
void Initialize(void)
{
    ...
    // save the disk name if specified
    ...
    // get the output directory for .wav file
    if(!cmdLine.GetOptString(_T("output"), g_szOutputDir, MAX_PATH))
    {
        StringCchCopyEx(g_szOutputDir, MAX_PATH, _T("\\release"), NULL, NULL, STRSAFE_NULL_ON_FAILURE);
    }
    ...
    if(!cmdLine.GetOptVal(_T("track"), &g_cTrackNo))
    {   
        g_cTrackNo = 1;
    }
    ...
    if(g_szDiskName[0]) 
    {
        LOG(_T("CDDATEST: Disk Device Name = %s"), g_szDiskName);
    }
    else 
    {
        LOG(_T("CDDATEST: Disk Device will be auto-detected"));
    }
    
    if(g_szProfile[0]) 
    {
        LOG(_T("CDDATEST: Storage Profile = %s"), g_szProfile);
    }

    if(g_szOutputDir[0]) 
    {
        LOG(_T("CDDATEST: .wav Output Directory = %s"), g_szOutputDir);
    }

    LOG(_T("CDDATEST: Will copy track #%u"), g_cTrackNo);

    if(g_fOpenAsStore)
    {
        LOG(_T("CDDATEST: Will open device using OpenStore() API"));
    }

    if(g_fOpenAsVolume)
    {
        LOG(_T("CDDATEST: Will open disk as a volume (\\CDROM Drive\\VOL:)"));
    }
    
    // open a global handle to any CDDA device present
    g_hDisk = OpenCDDAHandle();
}
...
SHELLPROCAPI ShellProc(UINT uMsg, SPPARAM spParam)
{
    ...
    case SPM_BEGIN_GROUP:
        ...
        g_pKato->BeginLevel(0, TEXT("BEGIN GROUP: FSACCESS.DLL"));
        break;
    ...
    case SPM_END_GROUP:
        ...
        g_pKato->EndLevel(TEXT("END GROUP: FSACCESS.DLL"));
        break;
