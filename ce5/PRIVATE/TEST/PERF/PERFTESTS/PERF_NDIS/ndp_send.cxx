...
BOOL SendIterationWithPeer(
   ...
   DWORD *pSendRecvTime, *pSendRecvIdleTime , *pPacketsSentRecv ,*pBytesSentRecv;
...
int SendPerf()
{
   ...
   DWORD packetSize;
   DWORD sendTime, sendIdleTime, packetsSent, bytesSent;
   DWORD receiveTime, receiveIdleTime, packetsReceived, bytesReceived;
	   ...
	   if (g_receivePeer) {
		   ...
		   do
		   {
			   ...
			   LogMsg(
				   ...
				   packetsSent, packetSize, g_poolSize, sendTime, bytesSent/1048576.0,
				   (bytesSent * 8.0 * 1000.0)/(sendTime*1048576.0), 
				   packetsSent * 1000.0/sendTime, (100.0 * sendIdleTime)/sendTime
				   );
			   LogMsg(
				   _T("Recv %6d packets : size %4d bytes : pool %d packets : ")
				   _T("%5d ms : %5.1f MB : %4.1f Mbps : %8.1f pps : %4.1f idle : ")
				   _T("%4.1f lost"), 
				   packetsReceived, packetSize, g_poolSize, receiveTime, 
				   bytesReceived/1048576.0, 
				   (bytesReceived * 8000.0)/(receiveTime*1048576.0), 
				   packetsReceived * 1000.0/receiveTime, 
				   (100.0*receiveIdleTime)/receiveTime, 
				   100.0 - (bytesReceived*100.0) /bytesSent
				   );
		   ...
		   LogMsg(
			   ...
			   packetsSent, packetSize, g_poolSize, sendTime, 
			   (bytesSent * 8.0 * 1000.0)/(sendTime*1048576.0), 
			   packetsSent * 1000.0/sendTime, (100.0 * sendIdleTime)/sendTime
			   );
