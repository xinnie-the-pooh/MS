...
TESTPROCAPI RasServerLineEnable(UINT uMsg, 
						 TPPARAM tpParam, 
...
{	
	...
	if(bExpectedFail)
	{
		...
		//
		dwResult = RasIOControl(NULL, RASCNTL_SERVER_LINE_ENABLE, NULL, 0, NULL, 0, &cbStatus);
	...
	switch(LOWORD(lpFTE->dwUserData))
	{
		...
		case RASSERVER_INVALID_ENABLED:
			dwResult = RasIOControl(NULL, RASCNTL_SERVER_LINE_ENABLE, (PUCHAR)&Line, sizeof(Line), NULL, 0, &cbStatus);
			if (dwResult != 0)
			{
				RasPrint(TEXT("Unable to enable a line for RASCNTL_SERVER_LINE_ENABLE"));
				return TPR_SKIP;
			}	
			break;

		case RASSERVER_VALID_ENABLED:
			break;
			
		case RASSERVER_NONEXISTENT_ENABLED:
