...
// CE uses DWORD address length to keep structure aligned for RISC CPUs
#ifdef UNDER_CE
typedef DWORD TA_ADDRESS_LEN;
#else
typedef USHORT TA_ADDRESS_LEN;
#endif

typedef UNALIGNED struct _TA_ADDRESS {
    TA_ADDRESS_LEN AddressLength;       // length in bytes of Address[] in this
...
typedef struct _TA_ADDRESS_NETBIOS {
    LONG TAAddressCount;
    struct _Addr {
        TA_ADDRESS_LEN AddressLength; // length in bytes of this address == 18
...
typedef struct _TA_ADDRESS_NETBIOS_EX {
    ...
    struct _AddrNetbiosEx {
        TA_ADDRESS_LEN AddressLength; // length in bytes of this address == 36
...
typedef struct _TA_APPLETALK_ADDR {
    LONG TAAddressCount;
    struct _AddrAtalk {
        TA_ADDRESS_LEN AddressLength; // length in bytes of this address == 4
...
typedef struct _TA_ADDRESS_IP {
    LONG TAAddressCount;
    struct _AddrIp {
        TA_ADDRESS_LEN AddressLength; // length in bytes of this address == 14
...
typedef struct _TA_ADDRESS_IP6 {
    LONG TAAddressCount;
    struct _AddrIp6 {
        TA_ADDRESS_LEN AddressLength; // length in bytes of this address == 24
...
typedef struct _TA_ADDRESS_IPX {
    LONG TAAddressCount;
    struct _AddrIpx {
        TA_ADDRESS_LEN AddressLength; // length in bytes of this address == 12
