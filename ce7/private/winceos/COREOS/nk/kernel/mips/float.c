...
#ifdef  _MIPS64
#define SREG_TYPE    LONGLONG
#else   //  _MIPS64
#define SREG_TYPE    ULONG
#endif  //  _MIPS64
...
--*/
    ...
    FP_DOUBLE_OPERAND DoubleOperand1 = {0};
    FP_DOUBLE_OPERAND DoubleOperand2 = {0};
    ...
    FP_SINGLE_OPERAND SingleOperand4 = {0};
    FP_DOUBLE_OPERAND DoubleOperand4 = {0};
#endif
    ULONG Function;
    ULONG Index;
    ...
    LONG SingleMantissa;
    FP_SINGLE_OPERAND SingleOperand1 = {0};
    FP_SINGLE_OPERAND SingleOperand2 = {0};
    ...
    __try {
        ...
        // Check for a CTC1 instruction, which can raise an exception.  Let the
        // exception propagate unmodified.  Software will receive the proper
        // exception code, but no parameters will be populated.

        if ((Format == FORMAT_CT) && (Function == 0) && (ContextBlock.Fd == 0)) {
            return FALSE;
        }

        //
        // Check for illegal register specification or format code.
        // NOTE: only MIPS64 uses odd-numbered registers (fr1, fr3, fr5, ...)
        //
            ...
            if (Format == FORMAT_SINGLE) {
                ...
                if ((DoubleOperand1.Nan != FALSE) &&
                    ...
                    return KiInvalidOperationDouble(&ContextBlock,
                       ...
                       (Function >= FLOAT_CONVERT_SINGLE)) {
                Longword = (LONG)KiGetRegisterValue(Fs + 32,
                                                    ContextBlock.pctx);
        ...
        ExceptionRecord->ExceptionCode = (DWORD) STATUS_ILLEGAL_INSTRUCTION;
        return FALSE;
...

{
    ...
    u.LargeValue.HighPart = DOUBLE_INFINITY_VALUE_HIGH | (ResultSign << 31);
    u.LargeValue.LowPart = DOUBLE_INFINITY_VALUE_LOW;
    ...
    if (DoubleOperand1->Infinity == FALSE) {
        ((PFSR)&ContextBlock->pctx->Fsr)->SZ = 1;
        if (((PFSR)&ContextBlock->pctx->Fsr)->EZ != 0) {
            ExceptionRecord->ExceptionCode = (DWORD) STATUS_FLOAT_DIVIDE_BY_ZERO;
            ((PFSR)&ContextBlock->pctx->Fsr)->XZ = 1;
            IeeeValue = (PFP_IEEE_VALUE)&ExceptionRecord->ExceptionInformation[2];
            IeeeValue->Value.Fp64Value.W[0] = u.LargeValue.LowPart;
            IeeeValue->Value.Fp64Value.W[1] = u.LargeValue.HighPart;
            return FALSE;
        }
    }

    KiSetRegisterDouble(ContextBlock->Fd + 32,
                        u.ResultValue,
                        ContextBlock->pctx);

    ContextBlock->pctx->Fir = ContextBlock->BranchAddress;
    return TRUE;
}
...

{
    ...
    if ((CheckForNan == FALSE) ||
        ...
        ((PFSR)&ContextBlock->pctx->Fsr)->SV = 1;
        if (((PFSR)&ContextBlock->pctx->Fsr)->EV != 0) {
            ExceptionRecord->ExceptionCode = (DWORD) STATUS_FLOAT_INVALID_OPERATION;
            ((PFSR)&ContextBlock->pctx->Fsr)->XV = 1;
            IeeeValue = (PFP_IEEE_VALUE)&ExceptionRecord->ExceptionInformation[2];
...

{
    ...
    if (DoubleOperand1->Nan != FALSE) {
        ...
        if ((DoubleOperand1->MantissaLow | MantissaHigh) != 0) {
            u.LargeValue.LowPart = DoubleOperand1->MantissaLow >> 2;
            u.LargeValue.LowPart |= DoubleOperand1->MantissaHigh << 30;
            u.LargeValue.HighPart = DoubleOperand1->MantissaHigh >> 2;
            u.LargeValue.HighPart |= DOUBLE_QUIET_NAN_PREFIX;
            u.LargeValue.HighPart &= ~DOUBLE_QUIET_NAN_MASK;

        } else {
            u.LargeValue.LowPart = DOUBLE_NAN_LOW;
            u.LargeValue.HighPart = DOUBLE_QUIET_NAN;
        }
        ...
        if ((DoubleOperand2->MantissaLow | MantissaHigh) != 0) {
            u.LargeValue.LowPart = DoubleOperand2->MantissaLow >> 2;
            u.LargeValue.LowPart |= DoubleOperand2->MantissaHigh << 30;
            u.LargeValue.HighPart = DoubleOperand2->MantissaHigh >> 2;
            u.LargeValue.HighPart |= DOUBLE_QUIET_NAN_PREFIX;
            u.LargeValue.HighPart &= ~DOUBLE_QUIET_NAN_MASK;
...

{
    ...
    PEXCEPTION_RECORD ExceptionRecord;
    ULONG ExceptionResultHigh = 0;
    ULONG ExceptionResultLow = 0;
    PFP_IEEE_VALUE IeeeValue;
    BOOLEAN Inexact;
    BOOLEAN Overflow;
    ULONG RoundBit;
    ...
    if (ResultOperand->Infinity != FALSE) {
        u.LargeValue.LowPart = DOUBLE_INFINITY_VALUE_LOW;
        u.LargeValue.HighPart = DOUBLE_INFINITY_VALUE_HIGH | (ResultOperand->Sign << 31);
    ...
    if (ResultOperand->Exponent >= DOUBLE_MAXIMUM_EXPONENT) {
        ...
        case ROUND_TO_ZERO:
            u.LargeValue.LowPart = DOUBLE_MAXIMUM_VALUE_LOW;
            u.LargeValue.HighPart =
        ...
        case ROUND_TO_PLUS_INFINITY:
            if (ResultOperand->Sign == 0) {
                u.LargeValue.LowPart = DOUBLE_INFINITY_VALUE_LOW;
                u.LargeValue.HighPart = DOUBLE_INFINITY_VALUE_HIGH;

            } else {
                u.LargeValue.LowPart = DOUBLE_MAXIMUM_VALUE_LOW;
                u.LargeValue.HighPart = (ULONG)(DOUBLE_MAXIMUM_VALUE_HIGH | (1 << 31));
            }
        ...
        case ROUND_TO_MINUS_INFINITY:
            if (ResultOperand->Sign != 0) {
                u.LargeValue.LowPart = DOUBLE_INFINITY_VALUE_LOW;
                u.LargeValue.HighPart = (ULONG)(DOUBLE_INFINITY_VALUE_HIGH | (1 << 31));

            } else {
                u.LargeValue.LowPart = DOUBLE_MAXIMUM_VALUE_LOW;
                u.LargeValue.HighPart = DOUBLE_MAXIMUM_VALUE_HIGH;
            }
        ...
        if ((ResultOperand->Exponent <= DOUBLE_MINIMUM_EXPONENT) &&
            ...
            if (((PFSR)&ContextBlock->pctx->Fsr)->FS == 0) {
                ...
                if (DenormalizeShift > 53) {
                    ...
                    u.LargeValue.LowPart = ResultOperand->MantissaHigh >> DenormalizeShift;
                    u.LargeValue.HighPart = 0;

                } else if (DenormalizeShift >= 32) {
                    DenormalizeShift -= 32;
                    if (DenormalizeShift == 0) {
                        StickyBits |= ResultOperand->MantissaLow & ((1UL << (32 - 1)) - 1);
                        RoundBit = ResultOperand->MantissaLow & (1UL << (32 - 1));
                    } else {
                        StickyBits |= ResultOperand->MantissaLow |
                            (ResultOperand->MantissaHigh & ((1 << (DenormalizeShift-1)) - 1));
                        RoundBit = ResultOperand->MantissaHigh & (1 << (DenormalizeShift - 1));
                    }
                    ...
                    StickyBits |=
                        ResultOperand->MantissaLow & ((1 << (DenormalizeShift-1)) - 1);
                    RoundBit = ResultOperand->MantissaLow & ( 1 << (DenormalizeShift - 1));
                    ...
                    u.LargeValue.LowPart = ResultOperand->MantissaLow;
                    u.LargeValue.HighPart = ResultOperand->MantissaHigh;
                }

                if ((1 - ResultOperand->Exponent) > 0) {
                    //
                    // Round the result value using the mantissa and the sticky bits.
                    //
                    switch (ContextBlock->Round) {
                    ...
                    case ROUND_TO_NEAREST:
                        if (RoundBit != 0) {
                            if ((StickyBits != 0) || ((ResultOperand->MantissaLow & 0x01) != 0)) {
                                u.LargeValue.LowPart += 1;
                                if (u.LargeValue.LowPart < 1) {
                                    u.LargeValue.HighPart += 1;
                                }
                            }
                        }
                        break;
                ...
                u.LargeValue.HighPart |= (ResultOperand->Sign << 31);
                if (StickyBits != 0) {
                    Inexact = TRUE;
                    Overflow = FALSE;
                    Underflow = TRUE;
            ...
            u.LargeValue.LowPart = ResultOperand->MantissaLow;
            u.LargeValue.HighPart = ResultOperand->MantissaHigh & ((1 << (52 - 32)) - 1);
            u.LargeValue.HighPart |= (ResultOperand->Exponent << (52 - 32));
            u.LargeValue.HighPart |= (ResultOperand->Sign << 31);
    ...
    if (Overflow != FALSE) {
        ((PFSR)&ContextBlock->pctx->Fsr)->SI = 1;
        ((PFSR)&ContextBlock->pctx->Fsr)->SO = 1;
        if ((((PFSR)&ContextBlock->pctx->Fsr)->EO != 0) ||
            (((PFSR)&ContextBlock->pctx->Fsr)->EI != 0)) {
            if (((PFSR)&ContextBlock->pctx->Fsr)->EO != 0) {
                ExceptionRecord->ExceptionCode = (DWORD) STATUS_FLOAT_OVERFLOW;

            } else {
                ExceptionRecord->ExceptionCode = (DWORD) STATUS_FLOAT_INEXACT_RESULT;
            }

            IeeeValue = (PFP_IEEE_VALUE)&ExceptionRecord->ExceptionInformation[2];
            IeeeValue->Value.Fp64Value.W[0] = ExceptionResultLow;
            IeeeValue->Value.Fp64Value.W[1] = ExceptionResultHigh;
            ((PFSR)&ContextBlock->pctx->Fsr)->XI = 1;
            ((PFSR)&ContextBlock->pctx->Fsr)->XO = 1;
            return FALSE;
        }

    } else if (Underflow != FALSE) {
        ((PFSR)&ContextBlock->pctx->Fsr)->SI = 1;
        ((PFSR)&ContextBlock->pctx->Fsr)->SU = 1;
        if ((((PFSR)&ContextBlock->pctx->Fsr)->EU != 0) ||
            (((PFSR)&ContextBlock->pctx->Fsr)->EI != 0)) {
            if (((PFSR)&ContextBlock->pctx->Fsr)->EU != 0) {
                ExceptionRecord->ExceptionCode = (DWORD) STATUS_FLOAT_UNDERFLOW;
            ...
            IeeeValue->Value.Fp64Value.W[1] = ExceptionResultHigh;
            ((PFSR)&ContextBlock->pctx->Fsr)->XI = 1;
            ((PFSR)&ContextBlock->pctx->Fsr)->XU = 1;
            return FALSE;
        }

    } else if (Inexact != FALSE) {
        ((PFSR)&ContextBlock->pctx->Fsr)->SI = 1;
        if (((PFSR)&ContextBlock->pctx->Fsr)->EI != 0) {
            ExceptionRecord->ExceptionCode = (DWORD) STATUS_FLOAT_INEXACT_RESULT;
            ((PFSR)&ContextBlock->pctx->Fsr)->XI = 1;
            IeeeValue = (PFP_IEEE_VALUE)&ExceptionRecord->ExceptionInformation[2];
            IeeeValue->Value.Fp64Value.W[0] = u.LargeValue.LowPart;
...

{
    ...
    //
    // If the result is neither zero nor denormal, round now.
    //
    ...
    if (ResultOperand->Exponent >= SINGLE_MAXIMUM_EXPONENT) {
        ...
        if ((ResultOperand->Exponent <= SINGLE_MINIMUM_EXPONENT) &&
            ...
            if (((PFSR)&ContextBlock->pctx->Fsr)->FS == 0) {
                ...
                StickyBits |= (ResultOperand->Mantissa & ((1 << (DenormalizeShift - 1)) - 1));
                RoundBit = ((ResultOperand->Mantissa & (1 << (DenormalizeShift - 1))) != 0);
                if ((StickyBits != 0) || (RoundBit != 0)) {
                    ...
                    //
                    // Rounding may have caused the mantissa to overflow into
                    // the exponent, resulting in a normalized number.  Do not
                    // signal underflow in this case.
                    //

                    Inexact = TRUE;
                    Overflow = FALSE;
                    Underflow = (ResultValue & (1 << 23)) ? FALSE : TRUE;
...

{
    ...
    u.ResultValue = KiGetRegisterDouble(Source + 32,
                                        ContextBlock->pctx);

    DoubleOperand->Sign = u.LargeValue.HighPart >> 31;
    DoubleOperand->Exponent = (u.LargeValue.HighPart >> (52 - 32)) & 0x7ff;
    DoubleOperand->MantissaHigh = u.LargeValue.HighPart & 0xfffff;
    DoubleOperand->MantissaLow = u.LargeValue.LowPart;
...

{
    ...
    Value = (ULONG)KiGetRegisterValue(Source + 32,
                                      ContextBlock->pctx);
...
ULONG KiEmulateBranch(PCONTEXT pctx) {
...
--*/
    ...
    BranchInstruction.Long = *((PULONG)pctx->Fir);
    ...
    BranchTaken = (pctx->Fir + 4) +
                             (LONG)(BranchInstruction.i_format.Simmediate << 2);
    BranchNotTaken = pctx->Fir + 8;
    ...
    case J_OP:
        return ((pctx->Fir + 4) & 0xf0000000) | (BranchInstruction.j_format.Target << 2);
...
Routine Description:
    ...
    This function is called to get the value of a register from the
...

{
    if (Register == 0)
        ...
        return (REG_TYPE)(&pctx->IntAt)[Register-1];
    else
        return (REG_TYPE)(&pctx->FltF0)[Register-32];
}
...
Routine Description:
    ...
    This function is called to set the value of a register in the
    specified exception or trap frame.
...

{
    if (Register > 0) {
        if (Register < 32)
            (&pctx->IntAt)[Register-1] = Value;
        else
            (&pctx->FltF0)[Register-32] = Value;
    }
}
...
Routine Description:
    ...
    This function is called to get the 64-bit value of a floating register from the
    specified exception or trap frame.

Arguments:

    Register - Supplies the number of the register whose value is to be
        returned. Floating registers are specified as 32 - 63, in MIPS32,
        we return Register and (Register + 1) to form a 64-bit value, in
        MIPS64, only return Register value.
...

{
    ...
    if (Register < 32)
        ...
        return (ULONGLONG)(&pctx->FltF0)[Register - 32];
#else
    union {
        ULONGLONG ResultValue;
        ULARGE_INTEGER LargeValue;
    ...
    if (Register < 32)
        ...
        u.LargeValue.LowPart  = (ULONG)(&pctx->FltF0)[Register - 32];
        u.LargeValue.HighPart = (ULONG)(&pctx->FltF0)[Register - 32 + 1];
    }
    return u.ResultValue;
#endif
}
...
Routine Description:
    ...
    This function is called to set the 64-bit value of a floating register in the
    specified exception or trap frame.

Arguments:

    Register - Supplies the number of the register whose value is to be
        stored. Floating registers are specified as 32 - 63, in MIPS32,
        we store 64-bit value to Register and (Register + 1).  In MIPS64,
        the 64-bit value is stored to Register only.
...
#ifdef _MIPS64
    if (Register >= 32) {
        (ULONGLONG)(&pctx->FltF0)[Register-32] = Value;
    }
#else
    if (Register >= 32) {
        (ULONG)(&pctx->FltF0)[Register-32] = (LONG)Value;
        (ULONG)(&pctx->FltF0)[Register-32+1] = (LONG)(Value >> 32);
    }
#endif
}
#endif
