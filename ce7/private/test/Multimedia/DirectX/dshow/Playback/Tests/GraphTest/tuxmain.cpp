...
GraphTest g_GraphTest;
FUNCTION_TABLE_ENTRY* g_lpFTE = NULL;

extern void SetKato(CKato * pKato);
...
SHELLPROCAPI ShellProc(UINT uMsg, SPPARAM spParam)
{
    ...
    case SPM_LOAD_DLL:
        ...
        SetKato(g_pKato);
	CVerificationMgr::GlobalInit();
        break;

    case SPM_UNLOAD_DLL:
        // Sent once to the DLL immediately before it is unloaded.
        Debug(TEXT("ShellProc(SPM_UNLOAD_DLL, ...) called"));
	CVerificationMgr::GlobalRelease();
        break;
    ...
    case SPM_SHELL_INFO:
        ...
        if(FAILED(g_GraphTest.ParseCommandLine(g_pShellInfo->szDllCmdLine)))
        {
        ...
        // if the process command line fails, then that means the user requested the command
        // line options, so exit the dll.
        if(FAILED(g_GraphTest.GetTuxFunctionTable(&g_lpFTE)))
        {
            Debug(TEXT("Unable to get the function table pointer."));
            return SPR_FAIL;
        }
    ...
    case SPM_REGISTER:
        ...
        // Build the function table and set it with the Shell
    ...
    case SPM_BEGIN_TEST:
        ...
        // Let the test graph know the ID so it can load the correct .GRF file if necessary
        pBT = (LPSPS_BEGIN_TEST)spParam;
        lpFTE = pBT->lpFTE;
        pTestDesc = (TestDesc*)lpFTE->dwUserData;
        TestGraph::SetGraphID(pTestDesc->testId);
