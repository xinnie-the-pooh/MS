...
{
    static  TCHAR   szHeader[] = TEXT("fsLargeFiles: ");
    TCHAR   szBuffer[1024];
...
{
    Debug(TEXT(""));
    Debug(TEXT("usage: tux -o -d fsLargeFiles -c \"/path <path>\" -x[test case number]"));
    Debug(TEXT("       /path default to \"\\Hard Disk\""));
    Debug(TEXT(""));
}
...
{
    StringCchCopy(g_szHardDiskRoot,MAX_PATH,HARD_DISK_ROOT);
    CClParse cmdLine(g_pShellInfo->szDllCmdLine);
    cmdLine.GetOptString(_T("path"),g_szHardDiskRoot,MAX_PATH);
}
