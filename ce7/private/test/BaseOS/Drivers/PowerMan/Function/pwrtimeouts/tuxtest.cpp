...
static BOOL ProcessCmdLine(LPCTSTR szCmdLine)
{
    ...
    BOOL bRet = FALSE; 
    UINT cmdLineLen = MAX_PATH;

    // case for zero command line arguments is valid
    if( SUCCEEDED(StringCchLength(szCmdLine, MAX_PATH, &cmdLineLen)) && (cmdLineLen == 0) )
    {   
        bRet = TRUE;
    }

    // check for a path
    if(!cmdLine.GetOptString(cmdProfile, g_szProfile, MAX_PATH))
    {
        g_pKato->Log( LOG_DETAIL, TEXT("No storage profile specified"));
        g_pKato->Log( LOG_DETAIL, TEXT("Default Storage Profile: \"%s\" will be used for external storage test case"), g_szProfile);
    }
    else
    {
       g_pKato->Log( LOG_DETAIL, TEXT("Storage Profile specified: \"%s\" "), g_szProfile);
       bRet = TRUE;
    }

    // check for enabling suspend/resume
    if(cmdLine.GetOpt(cmdAllowSuspend)) 
    {
        ...
        g_fAllowSuspend = TRUE;
        bRet = TRUE;
    }
    else
    {
        Debug(TEXT("Suspend disabled for Power Manager test."));
        g_fAllowSuspend = FALSE;

        // Disable suspend
        DisableSystemSuspend(TRUE);
    }

    // check for running on a PDA version of the power model
    ...
    if(!bRet)
    {
        ...
        {
            g_pKato->Log( LOG_FAIL, TEXT("FAIL: Command line options are not valid"));
        }
        Usage();
    }
...
{    
    Debug(TEXT(""));
    Debug(TEXT("Use this test suite to test timeout values for the Power Manager."));
    Debug(TEXT(""));
    Debug(TEXT("pwrtimeouts: Usage: tux -o -d pwrtimeouts -c\"<arguments>\""));
    Debug(TEXT("The tests take the following optional command line arguments:"));
    Debug(TEXT("       -Profile <Storage Profile>        : Storage Profile - E.g. FlashDisk; Optional; Default - SDMemory"));
    Debug(TEXT("       -AllowSuspend               : Specify this option (-c\"-AllowSuspend\") to allow the tests to suspend the system."));
}
