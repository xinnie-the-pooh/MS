...
BOOL ParseCmdLine(LPCTSTR szDllCmdLine)
{
    ...
    while ( szToken )
    {
        ...
        else if( wcsncmp( szToken, L"workdir=", 8 ) == 0 )
        {
            StringCchPrintfW( g_szPathRoot, 255, L"%s", szToken + 8 );
        }
        else if( wcsncmp( szToken, L"UseBaml", 7 ) == 0 )
        {
            g_bUseBaml = true;
        }
        else if( wcsncmp( szToken, L"data=", 5 ) == 0 )
        {
            StringCchPrintfW( g_szDataFilePath, 255, L"%s", szToken + 5 );
        }
...
SHELLPROCAPI ShellProc(UINT uMsg, SPPARAM spParam) {
   ...
   switch (uMsg) {
      ...
      case SPM_LOAD_DLL: {
        ...
        StringCchPrintfW( g_szPathRoot, 255, L"\\release" );
      ...
      case SPM_SHELL_INFO: {
         ...
         //get the size of the work area - for sizing scrolling areas in the test
        RECT Rect;
        SystemParametersInfo(SPI_GETWORKAREA, 0, &Rect, 0);

         g_ScrnHeight = Rect.bottom;
         g_ScrnWidth = Rect.right;

         //get screen resolution
        int h, w                    = 0;
        WCHAR szRight[MAX_PATH]     = {0};
        WCHAR szBottom[MAX_PATH]    = {0};
        WCHAR szRes[MAX_PATH]       = {0};

        h = GetSystemMetrics(SM_CYSCREEN);
        w = GetSystemMetrics(SM_CXSCREEN);
        _itow_s(w, szRight, 10);
        _itow_s(h, szBottom, 10);
        StringCchPrintfW( szRes, MAX_PATH, L"%sx%s", szRight, szBottom);
        g_pKato->Log(LOG_COMMENT, L"SPM_SHELL_INFO: Using resolution fit: %s", szRes);

        if (!LoadDataFile(g_szDataFilePath)) // load the xml data file once 
            return SPR_FAIL;
