...
BOOL ParseCmdLine(LPCTSTR szDllCmdLine)
{
    ...
    while ( szToken )
    {
        ...
        else if( wcsnicmp( szToken, L"MultiThread=", 12 ) == 0 )
        {
            if( *(szToken+12) == '1' )
            {
                g_bRunInMultiThreadMode = true;
            }
        }
...
SHELLPROCAPI ShellProc(UINT uMsg, SPPARAM spParam) {
   ...
   switch (uMsg) {
      ...
      case SPM_SHELL_INFO: {
        ...
        g_ScrnHeight = GetSystemMetrics(SM_CYSCREEN);
        g_ScrnWidth  = GetSystemMetrics(SM_CXSCREEN);
