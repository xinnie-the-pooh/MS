...
extern DWORD g_dwStressTime;
extern TCHAR g_strConfigFile[MAX_PATH];
extern TCHAR g_strTestHook[MAX_PATH];
extern BOOL  g_bUseTestHook;


static BOOL IsValidTime(DWORD *);
...
void Debug(LPCTSTR szFormat, ...)
{
    ...
    wcscat_s(szBuffer, COUNTOF(szBuffer), TEXT("\r\n"));
...
SHELLPROCAPI ShellProc(UINT uMsg, SPPARAM spParam)
{
    ...
    case SPM_LOAD_DLL:
        ...
        InitializeCriticalSection( &gI2CTestDataCriticalSection);
        InitializeCriticalSection( &gI2CTestInitCriticalSection);
        //I2CTestMutex= CreateMutex(NULL, TRUE, L"I2CTestMutex");
        break;
    ...
    case SPM_UNLOAD_DLL:
        ...
        //CloseHandle(I2CTestMutex);
        DeleteCriticalSection(&gI2CTestInitCriticalSection);
        DeleteCriticalSection(&gI2CTestDataCriticalSection);
        break;
    ...
    case SPM_SHELL_INFO:
        ...
        {
            g_pKato->Log( LOG_DETAIL, TEXT("Command Line Arguments: %s"), g_pShellInfo->szDllCmdLine );
            bValidCmd = ProcessCmdLine(g_pShellInfo->szDllCmdLine);

            if(!bValidCmd)
            {
                g_pKato->Log( LOG_ABORT, TEXT("Invalid Command Line"));
                Usage();
                return SPR_FAIL;
            }
        }
    ...
    case SPM_BEGIN_GROUP:
        ...
        // Load the test hook DLL

        if(g_bUseTestHook)
        {
            if(LoadI2CTestHook())
            {
                g_pKato->Log( LOG_DETAIL, TEXT("I2C Test Hook loaded"));
            }
            else
            {
                g_pKato->Log( LOG_DETAIL, TEXT("WARNING: Failed to load I2C Test Hook %s"), g_strTestHook);
                g_bUseTestHook = FALSE;
                break;
            }

            // Apply the I2C Test Hook
            if(ApplyTestHook())
            {
                g_pKato->Log( LOG_DETAIL, TEXT("I2C Test Hook applied"));
            }
            else
            {
                g_pKato->Log( LOG_DETAIL, TEXT("WARNING: Failed to apply I2C Test Hook"));
                g_bUseTestHook = FALSE;
            }
    ...
    case SPM_END_GROUP:
        ...
        if(g_bUseTestHook)
        {
            // Clear the I2C Test Hook
            if(ClearTestHook())
            {
                g_pKato->Log( LOG_DETAIL, TEXT("I2C Test Hook cleared"));
            }
            else
            {
                g_pKato->Log( LOG_DETAIL, TEXT("WARNING: Failed to clear I2C Test Hook"));
            }
        }
...
//-----------------------------------------------------------------------------
// Purpose:
//  Retrieves the value of command line parameters
// 
// Parameters:
// LPCTSTR szCmdLine - Command Line String
//
// Returns:
//      Return FALSE if any problems in parsing the command line, TRUE otherwise
//------------------------------------------------------------------------------
static BOOL ProcessCmdLine(LPCTSTR szCmdLine)
{
    ...
    if(!cmdLine.GetOptVal(_T("time"), (LPDWORD)&g_dwStressTime))
    {
        g_dwStressTime = DEFAULT_STRESS_TIME;
        g_pKato->Log( LOG_DETAIL, TEXT("StressTime not specified. Setting StressTime to %d"), g_dwStressTime);
    }
    else
    {
        g_dwStressTime = g_dwStressTime*60*1000; // ms
        if(!IsValidTime(&g_dwStressTime))
        {
            g_pKato->Log( LOG_FAIL, TEXT("StressTime %d invalid"), g_dwStressTime);
            bRet = FALSE;
            goto End;
        }
    }

    if(!cmdLine.GetOptString(_T("config"), g_strConfigFile, MAX_PATH))
    {
        StringCchCopy(g_strConfigFile, MAX_PATH, DEFAULT_I2C_CONFIG_FILE);
        g_pKato->Log( LOG_FAIL, TEXT("No config file specified. Setting config file to %s"), g_strConfigFile);
        
    }

    if(!cmdLine.GetOptString(_T("testhook"), g_strTestHook, MAX_PATH))
    {
        g_pKato->Log( LOG_DETAIL, TEXT("No test hook specified"));
        g_bUseTestHook = FALSE;
    }
    else
    {
        g_pKato->Log( LOG_DETAIL, TEXT("Test hook specified: %s "), g_strTestHook);
        g_bUseTestHook = TRUE;
    }
...
//-----------------------------------------------------------------------------
// Purpose:
//  Sets the default values for all the parameters if no command line params are specified
// 
// Parameters:
// None
//
...
{
    g_dwStressTime = DEFAULT_STRESS_TIME;
    g_pKato->Log( LOG_DETAIL, TEXT("StressTime not specified. Setting volume to %d"), g_dwStressTime);
...
//-----------------------------------------------------------------------------
// Purpose:
//  Checks whether the Stress Time is valid
// 
// Parameters:
// DWORD *in_pdwTime - StressTime
//
// Returns:
//      Return FALSE if not valid, TRUE otherwise
//------------------------------------------------------------------------------
static BOOL IsValidTime(DWORD *in_pdwTime)
{
    BOOL bRet = TRUE;
...
//-----------------------------------------------------------------------------
// Purpose:
//  Prints the usage of the tux command with the command line parameters
// 
// Parameters:
// None
//
...
{    
    Debug(TEXT(""));
    Debug(TEXT("testI2c: Usage: tux -o -d testi2c -c\" /time <Time in mins> /config <XML config file> /testhook <Test Hook DLL>\""));
    Debug(TEXT("       /time <Time>               : Time duration in mins for stress test; Optional; Default - 10 mins"));
    Debug(TEXT("       /config <XML config file>  : XML config file containing the I2C transaction details; Optional; Default - \\Release\\I2C_Test_Config.xml"));
    Debug(TEXT("       /testhook <Test Hook DLL>  : I2C Test Hook DLL; Optional"));
}
