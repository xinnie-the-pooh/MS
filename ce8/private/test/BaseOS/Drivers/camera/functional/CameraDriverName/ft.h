...
#define CONFORMANCE(x) PROPERTY_BASEID + x

#include <camera.h>

extern GUID g_CameraDevclass;
extern GUID g_CameraPinclass;
extern GUID g_3ptDevclass;

BEGIN_FTE
    FTH(0, "Driver Name testing")
    FTE(1,     "Enforce that all DShow based drivers are CAMX: devices", 100, (DWORD) &g_CameraDevclass, Test_DriverName )
    FTE(1,     "Enforce that all Pin members of DShow based drivers are PIN:", 101, (DWORD) &g_CameraPinclass, Test_DriverName )
    FTE(1,     "Enforce that all third party based drivers are CAPX: devices", 102, (DWORD) &g_3ptDevclass, Test_DriverName )
END_FTE
