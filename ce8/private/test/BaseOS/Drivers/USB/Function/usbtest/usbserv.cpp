...
DWORD WINAPI TransferNotify(LPVOID lpvNotifyParameter)
{
    ...
    if(pTransStatus->hTransfer)
    {
        if (!(*(pTransStatus->lpUsbFuncs->lpIsTransferComplete))(pTransStatus->hTransfer))
            pTransStatus->IsFinished=FALSE;

    }
    pTransStatus->cEvent.SetEvent();
    return 1;
}
...
USB_TRANSFER UsbClass::SetConfig(LPTRANSFER_NOTIFY_ROUTINE lpNodifyAddress,LPVOID lpNodifyParameter, DWORD dwFlags, UCHAR bType, UCHAR bIndex, WORD wLanguage,
                                          WORD wLength, LPVOID lpvBuffer)
{

//SetConfiguration is not Part of USBD
#if 0 
    return (*lpUsbFuncs->lpSetConfiguration)(hUsb,
            ((dwFlags & USB_NO_WAIT)!=0)?lpNodifyAddress:NULL,
            lpNodifyParameter,
            dwFlags,bType,bIndex,wLanguage,wLength,lpvBuffer);
#else
    UNREFERENCED_PARAMETER(lpNodifyAddress);
    UNREFERENCED_PARAMETER(lpNodifyParameter);
    UNREFERENCED_PARAMETER(dwFlags);
    UNREFERENCED_PARAMETER(bType);
...
BOOL UsbPipeService::ResetPipe()
{
    ...
    // Used to help synchronize new configuration reset and pipe reset
    Sleep(200); 
...
BOOL UsbPipeService::FormatData(LPBYTE pDataBuffer, DWORD dwDataLen)
{
    ...
    ASSERT(dwDataLen);
    WORD wSeqNumber=0;
    if (dwPacketSize>=4 && (dwPacketSize&3)==0 && pDataBuffer) {
