...
PDEVICE_SETTING InitializeSettings(){
    ...
    if(!dwBytesRet) {
        LOG(L"Can't get platform name length");
        goto EXIT;
    }

    dwOutBufSize = dwBytesRet;
    TCHAR *platName = (TCHAR *)malloc(dwBytesRet * sizeof(TCHAR));
    if(!platName) {
        LOG(L"Out of memory!");
        goto EXIT;
    }
    ...
    if (!KernelIoControl (IOCTL_HAL_GET_DEVICE_INFO, &dwInputSpi,
        sizeof(dwInputSpi), platName, dwOutBufSize, &dwBytesRet)) {
	    LOG(L"Can't call IOCTL_HAL_GET_DEVICE_INFO");
        free(platName);
        goto EXIT;
	}

    platName[dwOutBufSize/sizeof(TCHAR)] = L'\0';
    UCHAR endPointAttribOverride = 
        (_tcsstr(platName, L"OMAP35xx") != NULL) ? USB_ENDPOINT_TYPE_BULK : USB_ENDPOINT_TYPE_ISOCHRONOUS;
    free(platName);
    ...
    // --- high speed config ---
    // NOTE: if your device only support full/low speed, you still need to present high speed config,
    // the best way  to do it is to use the same configuation for high speed as full speed. You can set up
    // full speed configuration first, then just memcpy to everything to high speed configuration
    ...
    pCurEP->bmAttributes = endPointAttribOverride;//<--USER INPUT
    ...
    // --- set endpoints for full speed config 0, interface 0 ---
    //---endpoint 1---
