...
{    
    ...
    Debug(TEXT("       /disk <disk>        : name of the disk to test (e.g. DSK1:); default = all disks, skip bootable store."));
    Debug(TEXT("       /profile <profile>  : limit to devices of the specified storage profile; default = all disks, skip bootable store."));
    Debug(TEXT("       /store              : open the disk using the OpenStore() API"));
    Debug(TEXT("       /part               : open the disk using the OpenPartition() API"));
    ...
    switch (uMsg) {
        ...
        case SPM_SHELL_INFO: {
            ...
            if( g_pShellInfo->szDllCmdLine && *g_pShellInfo->szDllCmdLine )
            {
                ...
                {
                    g_pKato->Log( LOG_DETAIL, _T("WARNING:  Neither /disk or /profile is specified.\n"));
                    g_pKato->Log( LOG_DETAIL, _T("WARNING:  Thread x will test DSKx:\n"));
                    g_pKato->Log( LOG_DETAIL, _T("WARNING:  If there is only one thread, it will test the only DSKx: device. \n"));
                    g_pKato->Log( LOG_DETAIL, _T("WARNING:  This could be DSK1: or DSK2:, depending on the sequence of card inserts and removes. \n"));
