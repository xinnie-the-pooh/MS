...
extern DWORD             g_dwNumWrites;
extern DWORD             g_dwNumSectors;
extern DWORD             g_dwWriteSector;
            ...
            if( g_pShellInfo->szDllCmdLine && *g_pShellInfo->szDllCmdLine )
            {
                ...
                else
                {
                    if (g_szDiskName[0])
                        ...
                        g_hDisk = OpenFirstDetectedDiskHandleRW(g_pKato,g_fOpenAsStore,g_szDiskName,g_fOpenAsPartition);
                }
...
static void ProcessCmdLine(LPCTSTR szCmdLine)
{
   /*
    ...
    if(!cmdLine.GetOptVal(_T("threads"), &g_dwNumThreads))
    {
        g_dwNumThreads = NUMTHREADS;
    }
    if(!cmdLine.GetOptVal(_T("writes"), &g_dwNumWrites))
    {
        g_dwNumWrites = NUMWRITES;
    }
    if(!cmdLine.GetOptVal(_T("sectors"), &g_dwNumSectors))
    {
        g_dwNumSectors = NUMSECTORS ;
    }
    if(!cmdLine.GetOptVal(_T("writesector"), &g_dwWriteSector))
    {
        g_dwWriteSector = WRITESECTOR ;
    }
    g_bHelp = cmdLine.GetOpt(_T("?"));

}
static void Usage(void)
{    
    Debug(TEXT(""));
    Debug(TEXT("rwtest: Usage: tux -o -d rwtest -c\"-ZORCH /disk <disk> /threads <count> /writes <count> /sectors <count> /writesector <sector>\""));
    UsageCommon(_T("rwtest"),g_pKato);
    Debug(TEXT("       /threads <count>     : number of threads to use; default = (Hardcoded value from ft.h)"));
    Debug(TEXT("       /writes <count>      : number of writes to use; default = %u"),  NUMWRITES);
    Debug(TEXT("       /sectors <count>     : number of sectors to use; default = %u"), NUMSECTORS);
    Debug(TEXT("       /writesector <sector>: default = %u"), WRITESECTOR);
    Debug(TEXT("       /?                   : Show usage"));
}