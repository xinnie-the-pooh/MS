...
// file to match.
//
#define __BIN_NAME__    TEXT( "GDI_Detector.dll")
...
extern "C" {
    ...
    __BIN_NAME__, 0, 0, BVT_BASE+0, DetectorTest,  
    {NULL, 0, 0, 0, NULL}
    };
}
...
{
    BOOL bFlag = (WAIT_OBJECT_0 == WaitForAPIReady(SH_GDI, 0));

    // inform the logs if we found the technology
    if(bFlag)
        DETAIL("Successfully detected GDI");                      
    else
        DETAIL("Did not detect GDI");
