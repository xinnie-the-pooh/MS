...
HANDLE WINAPI STG_FindFirstStore( STOREINFO *pInfo, HANDLE hProc )
{
    ...
    DWORD   dwSize               = 0;
    BOOL    bSearchStoreMapAvail = FALSE;
    BOOL    bDeletepSearch       = FALSE;
    ...
    if (dwSize == sizeof(STOREINFO)) {
        ...
        if (pSearch) {
            bSearchStoreMapAvail = STG_CreateSearchStoreMap (pSearch);
            if(bSearchStoreMapAvail) {
                ...
                pSearch->DeletePartStoreInfoFromMap(pStorePartInfo->u.storeInfo.szDeviceName);
                pStorePartInfo = NULL;
            } else { //list empty
                bDeletepSearch = TRUE;
...
HANDLE WINAPI STG_OpenStoreEx( LPCTSTR szDeviceName, HANDLE hProc, DWORD dwAccess )
{
    ...
    GetShortDeviceName(szDeviceName, szShortDeviceName, DEVICENAMESIZE);
    LockStoreMgr( );
    pStore = GetStoreFromMap(szShortDeviceName);
    UnlockStoreMgr( );
