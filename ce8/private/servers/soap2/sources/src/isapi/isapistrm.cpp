...
BOOL COutputStream::SendHeader()
{
    ...
    case Error_AccessDenied:                            // permission denied
        ...
        stringChk = StringCchPrintfA(szTemp, _countof(szTemp), g_szHeaderExAccessDenied, szApplPath);
        ASSERT(SUCCEEDED(hr));
        pszHeaderEx = szTemp;
        break;
    ...
    if (m_cbFileSize != 0)
    {
        err = _ui64toa_s(m_cbFileSize, szLength, _countof(szLength), 10 );
        ASSERT(!err);
    }
    else
    {
        ...
        _itoa_s(cchContLength, szLength, _countof(szLength), 10);
    }

    {
        ...
        if (pszHeaderEx)
        {
            stringChk = StringCchCatNA(szHeaderTemp, _countof(szHeaderTemp), pszHeaderEx, MAX_HEADER_LENGTH - strlen(szHeaderTemp) - 3);
...
void  COutputStream::WriteFaultMessage(HRESULT hrResult, BSTR bstrActor)
{
    ...
    {
        HRESULT stringChk = StringCchCopyW((WCHAR *)tempbuf, cchTempbuf, g_pwstrISAPIErrorSource);
        ASSERT(SUCCEEDED(stringChk));
        stringChk = StringCchCatNW((WCHAR *)tempbuf, cchTempbuf, (WCHAR *)m_bstrErrorSource, FOURK-1-wcslen(g_pwstrISAPIErrorSource));
        ASSERT(SUCCEEDED(stringChk));
    }
    else
    {
    ...
    if (m_bstrErrorDescription.Len())
    {
        ...
        swprintf_s(&(tempbuf[0]), FOURK, L" HRESULT=0x%lX", hrResult);
...
HRESULT CSOAPIsapiResponse::put_HTTPStatus(BSTR bstrStatus)
{
...
#endif 
    {
        ...
        if (!pwstrStat)
            return E_OUTOFMEMORY;
        HRESULT hr;
        hr = StringCchCopyW (pwstrStat, wcslen(bstrStatus)+1, (WCHAR *) bstrStatus);
        ASSERT(SUCCEEDED(hr));
...
HRESULT CSOAPIsapiResponse::put_HTTPCharset(BSTR bstrCharset)
{
...
#endif 
    {
        ...
        if (!pwstrCharset)
            return E_OUTOFMEMORY;
        HRESULT hr;
        hr = StringCchCopyW (pwstrCharset, wcslen(bstrCharset)+1, (WCHAR *) bstrCharset);
        ASSERT(SUCCEEDED(hr));
