...
//-------------------------------------------------------------------------
// Class: 
// Method: Tc_APOFormatInt24wideFl32Create
//
// Description:
//  TestCase to test if AudioFormatInt24wideFl32 class is registered and the necessary interfaces can be obtained from it
...
{
    CAPOFormatInt24wideFl32Test APOFormatTest;
    return APOFormatTest.APOTestCreate(__uuidof(AudioFormatConvert));
}

//-------------------------------------------------------------------------
// Class: 
// Method: Tc_APOFormatInt24wideFl32Process
//
// Description:
//  TestCase to test if APO AudioFormatInt24wideFl32 behaves as expected
//  This is a global testcase function used in rgTestCaseInfo structure
// Parameters:
//
// Return values (DWORD):
...
//-------------------------------------------------------------------------
// Class: CAPOFormatInt24wideFl32Test
// Method: APOFormatProcess
//
// Description:
//   Functional testcase for FormatInt24wideFl32 APO
// Parameters:
//
// Return values (DWORD):
//     Return FNS_PASS on Success, FNS_FAIL on Failure
//
DWORD CAPOFormatInt24wideFl32Test::APOFormatProcess()
{
    HRESULT hr = S_OK;
    DWORD dw = FNS_PASS;
    ...
    METHOD_CHECKFAIL_SETVAL_EXIT(hr, "ERROR: APORT Object creation from AudioFormatInt24wideFl32 FAILED", dw, FNS_FAIL);
    
    SLOG(eInfo1, "Querying for IAPOConfig from FormatInt24wideFl32");
    ...
    INT32 iInputData[8]; //24bitwide is packed in the higher order bits of a 32 bit container    
    iInputData[0] = 0 << BYTE_SHIFT_COUNT;
    iInputData[1] = -0 << BYTE_SHIFT_COUNT;    
    iInputData[2] = -8388608 << BYTE_SHIFT_COUNT;
    iInputData[3] = 8388607 << BYTE_SHIFT_COUNT;
      ...
      )       
    {
        ...
        SLOG(eError, "Input 24bit: %d        Output Float: %e", (iInputData[0] >> BYTE_SHIFT_COUNT), fOutputData[0]);
        SLOG(eError, "Input 24bit: %d        Output Float: %e", (iInputData[1] >> BYTE_SHIFT_COUNT), fOutputData[1]);
        SLOG(eError, "Input 24bit: %d        Output Float: %e", (iInputData[2] >> BYTE_SHIFT_COUNT), fOutputData[2]);
        SLOG(eError, "Input 24bit: %d        Output Float: %e", (iInputData[3] >> BYTE_SHIFT_COUNT), fOutputData[3]);
        SLOG(eError, "Input 24bit: %d        Output Float: %e", (iInputData[4] >> BYTE_SHIFT_COUNT), fOutputData[4]);
        SLOG(eError, "Input 24bit: %d        Output Float: %e", (iInputData[5] >> BYTE_SHIFT_COUNT), fOutputData[5]);
        SLOG(eError, "Input 24bit: %d        Output Float: %e", (iInputData[6] >> BYTE_SHIFT_COUNT), fOutputData[6]);
        SLOG(eError, "Input 24bit: %d        Output Float: %e", (iInputData[7] >> BYTE_SHIFT_COUNT), fOutputData[7]);
        dw = FNS_FAIL;
        goto exit;
    }

    //Start with a INT24wide SineWave, Convert it to FLOAT32 and reconvert it to INT24wide
    ...
    // Call Int24wideFl32 collect the data in a temp buffer, then call Fl32Int24wide and collect the data in the 
    ...
    //At this point the sinewave has gone through a complete roundtrip from  INT24wide to FLOAT32 to INT24wide
