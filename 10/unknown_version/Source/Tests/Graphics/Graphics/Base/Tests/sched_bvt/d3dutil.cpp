                                 ...
                                 BOOL bAddWatermark )
{
    ...
    if( GetIconInfo( hCursor, &iconinfo ) )
    {
        if (0 != GetObject((HGDIOBJ)iconinfo.hbmMask, sizeof(BITMAP), (LPVOID)&bm))
        {
            ...
            if( SUCCEEDED(pd3dDevice->CreateOffscreenPlainSurface( dwWidth, dwHeightDest, 
                D3DFMT_A8R8G8B8, D3DPOOL_SCRATCH, &pCursorSurface, NULL ) ) )
            {
                pcrArrayMask = new DWORD[dwWidth * dwHeightSrc];
                if(NULL != pcrArrayMask)
                {
                    ...
                    if( hdcMask != NULL )
                    {
                        ...
                        if( SUCCEEDED(pd3dDevice->SetCursorProperties( iconinfo.xHotspot, 
