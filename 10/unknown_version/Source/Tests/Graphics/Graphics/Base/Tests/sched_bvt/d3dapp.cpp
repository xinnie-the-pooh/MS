...
HRESULT CD3DApplication::Create( HINSTANCE hInstance )
{
    ...
    #pragma prefast( suppress: 387, "m_hWnd cannot be null, it was just validated" );
...
bool CD3DApplication::FindBestWindowedMode( bool bRequireHAL, bool bRequireREF )
{
    ...
    for( UINT iai = 0; (iai < m_d3dEnumeration.m_pAdapterInfoList->Count()) && (!bFound); iai++ )
    {
        D3DAdapterInfo* pAdapterInfo = (D3DAdapterInfo*)m_d3dEnumeration.m_pAdapterInfoList->GetPtr(iai);
        for( UINT idi = 0; (idi < pAdapterInfo->pDeviceInfoList->Count()) && (!bFound); idi++ )
        {
            ...
            if (bRequireREF && pDeviceInfo->DevType != D3DDEVTYPE_REF)
                continue;
            for( UINT idc = 0; (idc < pDeviceInfo->pDeviceComboList->Count()) && (!bFound); idc++ )
            {
...
INT CD3DApplication::Run()
{
    ...
    STATE_TYPE stRequest = eStateRun; // default to run unless we're syncing (below)

    // Load keyboard accelerators
    HACCEL hAccel = LoadAccelerators( NULL, MAKEINTRESOURCE(IDR_MAIN_ACCEL) );

    // Here is where we sync to other applications if we need too
    if( m_bSyncWith )
    {
        pxProcSync = new CXProcSyncClient(m_wszSyncWith,m_nProcCount);
        if( pxProcSync )
        {
            // We're synchronizing cross process, we'll only want to work when the manager tells
            // us too. transition to ready.
    ...
    if( eStateRun == stRequest )
    {
        ...
        while( WM_QUIT != msg.message  )
        {
            ...
            // If we're syncing, we need to make sure that the manager wants us to keep working.
