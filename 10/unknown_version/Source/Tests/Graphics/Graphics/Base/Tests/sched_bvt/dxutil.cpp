...
HRESULT DXUtil_GetDXSDKMediaPathCch( TCHAR* strDest, int cchDest )
{
    ...
    #pragma prefast( suppress: 11, "strDest cannot be null, it was validated" );
    ...
    #pragma prefast( suppress: 387, "strDest cannot be null, it was validated" );
...
VOID DXUtil_LaunchReadme( HWND hWnd, TCHAR* strLoc )
{
    ...
    if( bFound )
    {
        ...
        #pragma prefast( suppress: 321, "Maybe we WANT to use a relative path name for a private shell32?" );
...
VOID DXUtil_Trace( TCHAR* strMsg, ... )
{
    ...
    _vsntprintf( strBuffer, 512-1, strMsg, args );
    strBuffer[512-1] = _T('\0');
    va_end(args);
