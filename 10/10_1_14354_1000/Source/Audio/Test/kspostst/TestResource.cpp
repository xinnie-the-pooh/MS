    ...
    )
{
    ...
    if (!VERIFY_IS_NOT_NULL(pHalf)){return E_INVALIDARG;}
    if (!VERIFY_IS_NOT_NULL(ppOut)){return E_POINTER;}

    pTestResource = new CPinTestResource();
    if (!VERIFY_IS_NOT_NULL(pTestResource)){return E_OUTOFMEMORY;}

    hr = VERIFY_SUCCEEDED_RETURN(pTestResource->Initialize(
            pHalf, guid, fLoopedStreaming));
    if(FAILED(hr))
    {
