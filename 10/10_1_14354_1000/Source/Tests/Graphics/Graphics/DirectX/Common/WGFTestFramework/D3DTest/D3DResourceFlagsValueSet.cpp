...
tstring __stdcall ToString( const D3DResourceFlags *pFlags )
{
	return	
		...
		ToString( (D3D_BIND_FLAG) pFlags->BindFlags ) + ", " +
		ToString( (D3D_CPU_ACCESS_FLAG) pFlags->CPUAccessFlags ) + ", " +
		ToString( (D3D_RESOURCE_MISC_FLAG) pFlags->MiscFlags ) + ", " +
		ToString( (D3D_MAP) pFlags->Map );
}
...
bool __stdcall FromString( D3DResourceFlags **ppFlags, const tstring &str )
{
	...
	FromString( (D3D_BIND_FLAG*) &(*ppFlags)->BindFlags, vStr[ 0 ] );
	FromString( (D3D_CPU_ACCESS_FLAG*) &(*ppFlags)->CPUAccessFlags, vStr[ 2 ] );
	FromString( (D3D_RESOURCE_MISC_FLAG*) &(*ppFlags)->MiscFlags, vStr[ 3 ] );
	FromString( (D3D_MAP*) &(*ppFlags)->Map, vStr[ 4 ] );
...
//{
//	//testfactor::RFactor rVBIBCopySrc = AddParameterValueSet< D3DResourceFlags* >( pSrcResourceFlags, pVBIBCopySrcValueSet );
//	return NULL;
//}
...
bool ValidD3DResourceFlags(D3D_RESOURCE_DIMENSION ResourceDimension, D3D_USAGE Usage, UINT BindFlags, UINT CPUAccessFlags, UINT MiscFlags, D3D_MAP Map, RESOURCE_ACCESS_METHOD AccessMethod, bool bGenerateMapCombos)
{
	if (CPUAccessFlags & D3D_CPU_ACCESS_WRITE)
	{
		//CPU Write only valid with Dynamic or Staging
		if (Usage != D3D_USAGE_DYNAMIC && Usage != D3D_USAGE_STAGING)
			return false;

		//CPU Write is invalid with GPU Output bind flags
		if (BindFlags & (D3D_BIND_STREAM_OUTPUT | D3D_BIND_RENDER_TARGET | D3D_BIND_DEPTH_STENCIL))
			return false;
	}

	if (CPUAccessFlags & D3D_CPU_ACCESS_READ)
	{
		//CPU Read only valid on Stating
		if (Usage != D3D_USAGE_STAGING)
			return false;
	...
	if (Usage == D3D_USAGE_DYNAMIC)
	{
		...
		if (BindFlags == 0 || (BindFlags != D3D_BIND_VERTEX_BUFFER && BindFlags != D3D_BIND_INDEX_BUFFER && BindFlags != D3D_BIND_CONSTANT_BUFFER && BindFlags != D3D_BIND_SHADER_RESOURCE) )
			return false;
	}
	...
	if (Usage == D3D_USAGE_IMMUTABLE)
	{
		...
		//Immutable needs to be used with at least one GPU Input bound flag
		if (0 == (BindFlags & (D3D_BIND_VERTEX_BUFFER | D3D_BIND_INDEX_BUFFER | D3D_BIND_CONSTANT_BUFFER | D3D_BIND_STREAM_OUTPUT)))
			return false;
	...
	if (BindFlags & (D3D_BIND_STREAM_OUTPUT | D3D_BIND_RENDER_TARGET | D3D_BIND_DEPTH_STENCIL))
	{
		...
		if (Usage != D3D_USAGE_DEFAULT)
			return false;
	}

	if (BindFlags & D3D_BIND_DEPTH_STENCIL)
	{
		//Can't have buffer or texture3d depthStencil
		if (ResourceDimension == D3D_RESOURCE_DIMENSION_BUFFER || ResourceDimension == D3D_RESOURCE_DIMENSION_TEXTURE3D )
			return false;
	}

	if (BindFlags & D3D_BIND_CONSTANT_BUFFER)
	{
	...
	if (ResourceDimension != D3D_RESOURCE_DIMENSION_BUFFER)
	{
		//Some bind flags are restricted to buffers
		if (BindFlags & (D3D_BIND_VERTEX_BUFFER | D3D_BIND_INDEX_BUFFER | D3D_BIND_CONSTANT_BUFFER | D3D_BIND_STREAM_OUTPUT))
			return false;
	}

	if (MiscFlags & D3D_RESOURCE_MISC_GENERATE_MIPS)
	{
	...
	//if (MiscFlags & D3D_RESOURCE_MISC_SHARED)
	//{
	//	//Shared resources not yet supported
	//	return false;
	//}

	if (MiscFlags & D3D_RESOURCE_MISC_TEXTURECUBE)
	{
		//Texture cubes needs to be 2D textures
		if ( ResourceDimension != D3D_RESOURCE_DIMENSION_TEXTURE2D )
			return false;
	}
	...
	if (bGenerateMapCombos)
	{
		if (Map != (D3D_MAP)0)
		{
			...
			if (Map == D3D_MAP_WRITE || Map == D3D_MAP_WRITE_DISCARD || Map == D3D_MAP_WRITE_NO_OVERWRITE || Map == D3D_MAP_READ_WRITE)
			{
				//Can't have write map access if no write CPUAccess
				if (0 == (CPUAccessFlags & D3D_CPU_ACCESS_WRITE))
					return false;
			}

			if (Map == D3D_MAP_READ || Map == D3D_MAP_READ_WRITE)
			{
				//Can't have read map access if no read CPUAccess
				if (0 == (CPUAccessFlags & D3D_CPU_ACCESS_READ))
					return false;
			}

			if (Map == D3D_MAP_WRITE_NO_OVERWRITE)
			{
				//Only VB and IB can use this write access
				if (0 == (BindFlags & (D3D_BIND_VERTEX_BUFFER | D3D_BIND_INDEX_BUFFER)))
					return false;
			}
			...
			if (Usage == D3D_USAGE_STAGING)
			{
				...
				if (Map != D3D_MAP_READ && Map != D3D_MAP_WRITE && Map != D3D_MAP_READ_WRITE)
					return false;
			}
		}
	...
	if (Usage == D3D_USAGE_IMMUTABLE || BindFlags & D3D_BIND_DEPTH_STENCIL)
	{
...
CD3DResourceFlagsValueSet::CD3DResourceFlagsValueSet( CTest *pTest, D3D_RESOURCE_DIMENSION ResourceDimension, bool bValid, bool bGenerateMapCombos, D3DResourceFlags *pIncludedFlags, D3DResourceFlags *pExcludedFlags, RESOURCE_ACCESS_METHOD AccessMethod ) :
		...
		m_bValid( bValid )
{
	...
	if (!pIncludedFlags)
	{
		m_IncludedFlags.Usage = (D3D_USAGE)DO_NOT_FILTER_FLAG;
		...
		m_IncludedFlags.Map = (D3D_MAP)DO_NOT_FILTER_FLAG;
	}
	else
	{
	...
	if (!pExcludedFlags)
	{
		m_ExcludedFlags.Usage = (D3D_USAGE)DO_NOT_FILTER_FLAG;
		...
		m_ExcludedFlags.Map = (D3D_MAP)DO_NOT_FILTER_FLAG;
	}
	else
	{
	...
	m_pUsageValueSet = new CTableValueSet<D3D_USAGE>( &m_ArrayD3DResourceFlags[0].Usage, sizeof( D3DResourceFlags ), m_NumD3DResourceFlags );
	m_pBindFlagsValueSet = new CTableValueSet<D3D_BIND_FLAG>( (D3D_BIND_FLAG*)&m_ArrayD3DResourceFlags[0].BindFlags, sizeof( D3DResourceFlags ), m_NumD3DResourceFlags );
	m_pCPUAccessFlagsValueSet = new CTableValueSet<D3D_CPU_ACCESS_FLAG>( (D3D_CPU_ACCESS_FLAG*)&m_ArrayD3DResourceFlags[0].CPUAccessFlags, sizeof( D3DResourceFlags ), m_NumD3DResourceFlags );
	m_pMiscFlagsValueSet = new CTableValueSet<D3D_RESOURCE_MISC_FLAG>( (D3D_RESOURCE_MISC_FLAG*)&m_ArrayD3DResourceFlags[0].MiscFlags, sizeof( D3DResourceFlags ), m_NumD3DResourceFlags );
	m_pMapValueSet = new CTableValueSet<D3D_MAP>( &m_ArrayD3DResourceFlags[0].Map, sizeof( D3DResourceFlags ), m_NumD3DResourceFlags );
}
...
void CD3DResourceFlagsValueSet::GenerateD3DResourceFlags()
{
	...
	bindFlags[D3D_BIND_VERTEX_BUFFER] = ToString(D3D_BIND_VERTEX_BUFFER);
	bindFlags[D3D_BIND_INDEX_BUFFER] = ToString(D3D_BIND_INDEX_BUFFER);
	bindFlags[D3D_BIND_CONSTANT_BUFFER] = ToString(D3D_BIND_CONSTANT_BUFFER);
	bindFlags[D3D_BIND_SHADER_RESOURCE] = ToString(D3D_BIND_SHADER_RESOURCE);
	bindFlags[D3D_BIND_STREAM_OUTPUT] = ToString(D3D_BIND_STREAM_OUTPUT);
	bindFlags[D3D_BIND_RENDER_TARGET] = ToString(D3D_BIND_RENDER_TARGET);
	bindFlags[D3D_BIND_DEPTH_STENCIL] = ToString(D3D_BIND_DEPTH_STENCIL);
	...
	miscFlags[D3D_RESOURCE_MISC_GENERATE_MIPS] = ToString(D3D_RESOURCE_MISC_GENERATE_MIPS);
	miscFlags[D3D_RESOURCE_MISC_SHARED] = ToString(D3D_RESOURCE_MISC_SHARED);
	miscFlags[D3D_RESOURCE_MISC_TEXTURECUBE] = ToString(D3D_RESOURCE_MISC_TEXTURECUBE);
	...
	CPUAccessFlags[D3D_CPU_ACCESS_WRITE] = ToString(D3D_CPU_ACCESS_WRITE);
	CPUAccessFlags[D3D_CPU_ACCESS_READ] = ToString(D3D_CPU_ACCESS_READ);
	...
	for (UINT Usage = D3D_USAGE_DEFAULT; Usage <= D3D_USAGE_STAGING; Usage++)
	{
		if( ( ( m_IncludedFlags.Usage != (D3D_USAGE)DO_NOT_FILTER_FLAG ) && ( m_IncludedFlags.Usage != Usage ) ) ||
			( ( m_ExcludedFlags.Usage != (D3D_USAGE)DO_NOT_FILTER_FLAG ) && ( m_ExcludedFlags.Usage == Usage ) ) )
			continue;
			...
			for (UINT iCPU=0; iCPU<CPUAccessFlagCombo.Count(); iCPU++)
			{
				...
				for (UINT iMisc=0; iMisc<miscFlagCombo.Count(); iMisc++)
				{
					...
					if( ( ( m_IncludedFlags.MiscFlags != DO_NOT_FILTER_FLAG ) && !( m_IncludedFlags.MiscFlags & MiscFlags ) ) ||
						...
						continue;
					for (UINT Map = (D3D_MAP)0; Map <= D3D_MAP_WRITE_NO_OVERWRITE; Map++)
					{
						if( ( ( m_IncludedFlags.Map != (D3D_MAP)DO_NOT_FILTER_FLAG ) && ( m_IncludedFlags.Map != Map ) ) ||
							( ( m_ExcludedFlags.Map != (D3D_MAP)DO_NOT_FILTER_FLAG ) && ( m_ExcludedFlags.Map == Map ) ) )
							continue;

						if (!m_bGenerateMapCombos && Map > 0)
							continue;

						if (m_bValid == ValidD3DResourceFlags(m_ResourceDimension, (D3D_USAGE)Usage, BindFlags, CPUAccessFlags, MiscFlags, (D3D_MAP)Map, m_AccessMethod, m_bGenerateMapCombos))
						{
							assert(m_NumD3DResourceFlags < 1024);

							D3DResourceFlags *pFlags = &m_ArrayD3DResourceFlags[m_NumD3DResourceFlags];
							pFlags->Usage = (D3D_USAGE)Usage;
							pFlags->BindFlags = BindFlags;
							pFlags->CPUAccessFlags = CPUAccessFlags;
							pFlags->MiscFlags = MiscFlags;
							pFlags->Map = (D3D_MAP)Map;
...
testfactor::RFactor CD3DResourceFlagsValueSet::CreateFactor( CTestCaseParameter<D3D_USAGE> *pUsageParam, CTestCaseParameter<D3D_BIND_FLAG> *pBindFlagsParam, CTestCaseParameter<D3D_CPU_ACCESS_FLAG> *pCPUAccessFlagsParam, CTestCaseParameter<D3D_RESOURCE_MISC_FLAG> *pMiscFlagsParam, CTestCaseParameter<D3D_MAP> *pMapParam )
{
	//TODO: if param is null, then don't create the factor and don't '%' it
	testfactor::RFactor rUsage = m_pTest->AddParameterValueSet<D3D_USAGE>(pUsageParam, m_pUsageValueSet);
	testfactor::RFactor rBindFlags = m_pTest->AddParameterValueSet<D3D_BIND_FLAG>(pBindFlagsParam, m_pBindFlagsValueSet);
	testfactor::RFactor rCPUAccessFlags = m_pTest->AddParameterValueSet<D3D_CPU_ACCESS_FLAG>(pCPUAccessFlagsParam, m_pCPUAccessFlagsValueSet);
	testfactor::RFactor rMiscFlags = m_pTest->AddParameterValueSet<D3D_RESOURCE_MISC_FLAG>(pMiscFlagsParam, m_pMiscFlagsValueSet);
	testfactor::RFactor rMap = m_pTest->AddParameterValueSet<D3D_MAP>(pMapParam, m_pMapValueSet);
