...
float Bias[] = { 5.f, 2.f, 1.f, 0.f, -1.f, -2.f, -5.f };

CBiasTest::CBiasTest()
{
	m_szTestName = "Bias Test";
	m_szCommandKey = "Bias";
}
...
UINT CBiasTest::TestInitialize(void)
{
	HRESULT hr;
	...
	//check for D3DSAMP_MIPMAPLODBIAS support
	if ( !(caps.RasterCaps & D3DPRASTERCAPS_MIPMAPLODBIAS) )
	{
        WriteToLog("CBiasTest: D3DSAMP_MIPMAPLODBIAS is not supported.\n");
        return D3DTESTINIT_SKIPALL;
    }
	
	tcCount.View       = m_nViewMatrices + 1; 
	tcCount.Bias       = sizeof(Bias       ) / sizeof(float);
	...
	for( tc.MagFilter = 0; tc.MagFilter < tcCount.MagFilter; tc.MagFilter++ )
	{
		...
		for( tc.MinFilter = 0; tc.MinFilter < tcCount.MinFilter; tc.MinFilter++ )
		{
			...
			{
				//D3DTEXF_ANISOTROPIC filter mode is not supported.
				//TEXF_FLATCUBIC and TEXF_GAUSSIANCUBIC are not supported for mipFilters
				if( /*Filters[ tc.MipFilter ] == TEXF_NONE || */Filters[ tc.MipFilter ] == TEXF_ANISOTROPIC )
					continue;
				...
				for(tc.LookupType = 0; tc.LookupType < tcCount.LookupType; tc.LookupType++)
				{
					...
					for( tc.Bias = 0; tc.Bias < tcCount.Bias; tc.Bias++ )
					for( tc.MipLevel = 0; tc.MipLevel < tcCount.MipLevel/*(tc.LookupType == PRESAMPLED ? 1 : tcCount.MipLevel)*/; tc.MipLevel++ )
					{
						...
						if (tc.LookupType == PRESAMPLED || KeySet(_T("fixed")))
							AddTestRange( tc.nTest + 2, tc.nTest + 2);//tcCount.View );
						else
							AddTestRange( tc.nTest + 1, tc.nTest + 2);//tcCount.View );
					}
				}
			}
		}
...
bool CBiasTest::ExecuteTest(UINT iTest)
{
	BIASTESTCASE tc;
	tc.nTest = iTest-1;
	...
	//AdaptiveTess_w = 1 << (int)( max(0, min(tcCount.MipLevel-1, tc.MipLevel+Bias[tc.Bias])) );
	//m_pRefDevice->SetRenderState(D3DRS_ADAPTIVETESS_W, *(DWORD*)&AdaptiveTess_w);
	...
	{
		m_TessLevel = (1 << tc.MipLevel)*min(m_MaxTess, (tc.MipLevel+1.f) );
		SetNPatchMode(m_TessLevel);
		SetRenderState(D3DRS_MAXTESSELLATIONLEVEL, *(DWORD*)&m_TessLevel);
		m_TessLevel = min(m_MaxTess, (tc.MipLevel+1.f) );
	}
		 ...
		 (m_pTexture == NULL || m_pDMTexture == NULL) )
	{
		...
		else
		{
			...
			if (tc.View > 1)
			{
				...
				float newBias = min(tcCount.MipLevel-1, Bias[tc.Bias]+tc.MipLevel);
				SetSamplerState( 0, SAMP_MAXMIPLEVEL, 0);
				SetSamplerState( 0, SAMP_MIPMAPLODBIAS, *(DWORD*)&newBias);
				SetSamplerState(DMAPSAMPLER, SAMP_MIPMAPLODBIAS, *(DWORD*)&Bias[tc.Bias]);
				//m_pRefDevice->SetSamplerState( 0, SAMP_MAXMIPLEVEL, (DWORD)newBias);
				//m_pRefDevice->SetSamplerState( 0, SAMP_MIPMAPLODBIAS, 0);
	...
	if (tc.View == 0)
	{
		...
		{
			SetSamplerState(0, SAMP_MIPMAPLODBIAS, *(DWORD*)&(Bias[tc.Bias]));
			SetSamplerState(0, SAMP_MAXMIPLEVEL, (DWORD)0);
			//m_pRefDevice->SetSamplerState(0, SAMP_MAXMIPLEVEL, (DWORD)min(tcCount.MipLevel-1, max(0, Bias[tc.Bias]+tc.MipLevel)));
			if ( min(7, max(0, Bias[tc.Bias]+tc.MipLevel)) < tc.MipLevel )
			{
				float x = min(7, max(0, Bias[tc.Bias]+tc.MipLevel)) - tc.MipLevel;
				SetSamplerState(0, SAMP_MIPMAPLODBIAS, *(DWORD*)&x);
			}
	...
	TCHAR tempStr[80];
	sprintf( tempStr, TEXT("%s, %d mip, %.1f bias, %s mag, %s min, %s mip"),
		LookupStr[tc.LookupType], tc.MipLevel, Bias[tc.Bias], 
