...
UINT CNewNTest::TestInitialize(void)
{
	HRESULT hr;
...
bool CNewNTest::ExecuteTest(UINT iTest)
{
	NEWTESTCASE tc;
	tc.nTest = iTest-1;
	...
	if (!KeySet("!c1"))
	if ( !DrawPrimitiveUP( D3DPT_TRIANGLEFAN, m_NumTri, m_pVCircle, sizeof(VERTEX)) )
...
bool CNewNTest::GenerateBuffers(float TessLevel)
{
	VOID   *pV = NULL;
	...
	VERTEX VCircle[4];
	VCircle[0] = VERTEX(D3DXVECTOR3( -cosPI_4, sinPI_4, 0.f ), D3DXVECTOR3( -cosPI_4, sinPI_4, 0.f ), 0.f, 0.f); 
	VCircle[1] = VERTEX(D3DXVECTOR3( -cosPI_4, -sinPI_4, 0.f  ), D3DXVECTOR3( -cosPI_4, -sinPI_4, 0.f ), 0.f, 0.f); 
	VCircle[2] = VERTEX(D3DXVECTOR3( cosPI_4, -sinPI_4, 0.f ), D3DXVECTOR3( cosPI_4, -sinPI_4, 0.f ), 0.f, 0.f); 
	VCircle[3] = VERTEX(D3DXVECTOR3( cosPI_4, sinPI_4, 0.f ), D3DXVECTOR3( cosPI_4, sinPI_4, 0.f ), 0.f, 0.f); 

	delete [] m_pVCircle;
	m_pVCircle = NULL;
	m_pVCircle = new VERTEX[(UINT)TessLevel + 2];

	int index = 0;
	float delta = (2.f*D3DX_PI)/TessLevel;
	m_pVCircle[index++] =VERTEX(D3DXVECTOR3( 0.f, 0.f, 0.f ), D3DXVECTOR3( 0.f, 0.f, 1.f ), 0.f, 0.f);
	for(int j = 0; j < 4; j++)
	{
		m_pVCircle[index++] = VCircle[j];
		float cosI = VCircle[j].position.x;
		float sinI = VCircle[j].position.y;
		for (int i= 1; i < TessLevel/4; i++)
		{
			cosI = VCircle[j].position.x*cos(i*delta) - VCircle[j].position.y*sin(i*delta); 
			sinI = VCircle[j].position.x*sin(i*delta) + VCircle[j].position.y*cos(i*delta);
			m_pVCircle[index++] = VERTEX(D3DXVECTOR3( cosI, sinI, 0.f ), D3DXVECTOR3( cosI, sinI, 0.f ), 0.f, 0.f); 
		}
	}
	m_pVCircle[index] = VCircle[0];
	m_NumTri = (DWORD)TessLevel; 
...
bool CNewNTest::GenerateVertexShader()
{
	HRESULT hr;
...
void CNewNTest::TransformVertex(D3DXMATRIX Matrix, D3DXVECTOR3 *Vertices, D3DXVECTOR3 *NewVertices, DWORD Size)
{
	float x, y, z, w;
...
bool CNewNTest::ProcessFrame(void)
{
...
bool CNewNTest::Setup()
{
	...
	{
		WriteToLog("CNewNTest::Setup - GenerateVertexShader failed. hr=%s.\n", m_pD3D->HResultToString(GetLastError()));
	}
	
	if (!GenerateBuffers(64))
		return false;
...
void CNewNTest::SetNewMaterial(UINT index)
{
