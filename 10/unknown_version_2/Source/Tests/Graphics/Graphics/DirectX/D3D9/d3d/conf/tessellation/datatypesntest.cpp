...
CDataTypesNTest::CDataTypesNTest()
{
	m_szTestName = "Data Types N Test";
	m_szCommandKey = "DataTypesN";

	m_nTessLevels = 3;//sizeof(DataTypesNTessLevels) / (sizeof(float));

	m_pVB = NULL;
	m_pIB = NULL;
	m_pShader = NULL;
	m_pDecl = NULL;
	m_dwStateBlockHandles = NULL;
}

UINT CDataTypesNTest::TestInitialize(void)
{
	HRESULT hr;
	CAPS caps;
	m_pSrcDevice->GetDeviceCaps(&caps);
  	m_nDataTypes = sizeof(DataTypesN) / sizeof(D3DDECLTYPE);
	...
	{
		if ( DataTypesN[tc.DataType] == D3DDECLTYPE_UBYTE4 && !(caps.DeclTypes & D3DDTCAPS_UBYTE4) )
			continue;
...
bool CDataTypesNTest::ExecuteTest(UINT iTest)
{
	DATATESTCASE tc;
	tc.nTest = iTest-1;
	...
	if ( tc.View == 0 || iTest == m_pRangeList->uStart || (m_pConstraints != NULL && iTest == m_pConstraints->uStart))
	{
		...
		sprintf( msgString, TEXT("%d patches, w=%d, h=%d, Tess=%.2f, %s"),
            NumPatches, m_uPatchWidth, m_uPatchHeight,
			m_TessLevels[tc.TessLevels][0], DataTypesNStr[tc.DataType]
		);
	}
	...
	ApplyStateBlock(m_dwStateBlockHandles[tc.TessLevels]);
	newValue = GetNPatchMode();
	...
	else if ( prim == D3DPT_TRIANGLELIST )
	{
		for (int j = 0; j < NumPatches; j++)
		{
			...
			//if ( !DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, m_uPatchWidth*m_uPatchHeight, j*6*(m_uPatchWidth-1)*(m_uPatchHeight-1), 2*(m_uPatchWidth-1)*(m_uPatchHeight-1), 0, CD3D_BOTH) )
			if ( !DrawIndexedPrimitive(D3DPT_TRIANGLELIST, MinIndex, NumVertices, StartIndex, PrimitiveCount, 0, CD3D_BOTH) )
			
				WriteToLog("%d DrawIndexedPrimitve failed.\n", iTest);
			//DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 6*(m_uPatchWidth-1)*(m_uPatchHeight-1), j*6*(m_uPatchWidth-1)*(m_uPatchHeight-1), 2*(m_uPatchWidth-1)*(m_uPatchHeight-1), 0, CD3D_BOTH);
			//DrawPrimitive(D3DPT_TRIANGLESTRIP, j*(m_uPatchWidth*2), m_uPatchWidth*2-2, CD3D_BOTH); 
		}
	}
...
bool CDataTypesNTest::GenerateBuffers(DATATESTCASE tc, D3DPRIMITIVETYPE primType)
{
	PBYTE pBuffer2 = NULL;
	VOID *pV = NULL;
	...
	ASSERT(0 !=m_uBufferHeight);
	ASSERT(0 !=m_uBufferWidth);
	switch ( DataTypesN[tc.DataType] ) 
	{
	...
	if( !CreateVertexBuffer( m_uBufferHeight*m_uBufferWidth*m_StreamSize, D3DUSAGE_NPATCHES, 0, POOL_MANAGED, &m_pVB )) 
	{
	...
	//if (!m_pIB->Lock( 0, m_nVertices*sizeof(WORD), (BYTE**)&pI/**temp** &pBuffer2*/, 0))
	{
	...
	switch (primType)
	{
				...
				for (int j = 0; j < m_uPatchHeight; j++)
				{
					...
					for (int i = 0; i < m_uPatchWidth; i++)
					{
						...
						if ( j > 0 ) //triangles on the top
						{
							if (i > 0)
							{
								tempI[ offset1 + 6*(i-1) + 1] = index;
								tempI[ offset1 + 6*(i-1) + 3] = index;
							}
							if (i < m_uPatchWidth - 1)
							{
								tempI[ offset1 + 6*i + 2] = index;
							}
						}
						if ( j < m_uPatchHeight - 1 ) //triangles on the bottom
						{
							...
							{
								tempI[ offset2 + 6*(i-1) + 5] = index;
							}
							if (i < m_uPatchWidth - 1) //triangle on the right
							{
								tempI[ offset2 + 6*i + 0] = index;
								tempI[ offset2 + 6*i + 4] = index;
							}
						}
						index++;
					}
		...
		case D3DPT_TRIANGLESTRIP:
		{
			for (UINT PatchInd = 0; PatchInd < m_nPatchesPerRow*m_nPatchesPerRow; PatchInd++)
			{
				...
				for (int i = 0; i < m_uPatchWidth; i++)
				{
					tempI[ offset + 2*i+0 ] = x_offset + i + (y_offset+1) * m_uBufferWidth;
					tempI[ offset + 2*i+1 ] = x_offset + i + (y_offset+0) * m_uBufferWidth;
				}
			}
			break;
		}
		case D3DPT_TRIANGLEFAN:
		{
			for (UINT PatchInd = 0; PatchInd < m_nPatchesPerRow*m_nPatchesPerRow; PatchInd++)
			{
				...
				x_offset = (PatchInd % m_nPatchesPerRow)*(m_uPatchWidth  - 1);
				y_offset = (PatchInd / m_nPatchesPerRow)*(m_uPatchHeight - 1);
				tempI[ offset ] = x_offset + m_uPatchWidth/2 + (y_offset + m_uPatchHeight/2) * m_uBufferWidth;
				for (int i = 0; i < m_uPatchWidth; i++)
				{
					tempI[ offset + 1 + i ] = x_offset + i + y_offset * m_uBufferWidth;
					tempI[ offset + 2*(m_uPatchWidth-1) + m_uPatchHeight - i ] = x_offset + i + (y_offset + m_uPatchHeight - 1) * m_uBufferWidth;
				}
				for (int j = 1; j < m_uPatchHeight - 1; j++)
				{
					tempI[ offset + m_uPatchWidth + j ] = x_offset + m_uPatchWidth - 1 + (j + y_offset) * m_uBufferWidth;
					tempI[ offset + 2*(m_uPatchWidth + m_uPatchHeight - 2) + 1 - j ] = x_offset + (j + y_offset) * m_uBufferWidth;
				}
				tempI[ offset + 2*(m_uPatchWidth + m_uPatchHeight - 2) + 1 ] = x_offset + y_offset * m_uBufferWidth;
			}
			break;
		}
	}
	memcpy( pIndices, tempI, m_nVertices*sizeof(WORD) ); //**temp**/
	//memcpy( pI, tempI, m_nVertices*sizeof(WORD));
	if (!m_pIB->Unlock())
	{
...
bool CDataTypesNTest::GenerateVertexShader(DATATESTCASE tc)
{
	HRESULT hr;
	...
	switch ( DataTypesN[tc.DataType] )
	{
		...
		{
			float value[4] = { 2.f, -1.f, 0.f, 0.f };
			if ( !SetVertexShaderConstantF( 16, value, 1 ))
				WriteToLog("GenerateBuffers - SetVertexShaderConstantF: Failed to set new value for c8.\n");

			VERTEXELEMENT decl[] = 
			{
				{0, 0, (DECLTYPE)D3DDECLTYPE_UBYTE4N,   DECLMETHOD_DEFAULT, DECLUSAGE_POSITION, 0},
				{0, 4, (DECLTYPE)D3DDECLTYPE_UBYTE4N,   DECLMETHOD_DEFAULT, DECLUSAGE_NORMAL,   0},
				{0, 8, (DECLTYPE)D3DDECLTYPE_UBYTE4N,   DECLMETHOD_DEFAULT, DECLUSAGE_TEXCOORD, 0},
				//{0, 32, (DECLTYPE)D3DDECLTYPE_D3DCOLOR, DECLMETHOD_DEFAULT, DECLUSAGE_DIFFUSE,  0},
				DECL_END()
			};
			if(!CreateVertexDeclaration( decl, &(m_pDecl)))
...
bool CDataTypesNTest::CapsCheck()
{
	...
	m_TessLevels[1][0] = m_MaxTess/2.f;
	m_TessLevels[2][0] = m_MaxTess;
	
	return true;
}

bool CDataTypesNTest::Setup()
{
	Tessellation::Setup();

	m_dwStateBlockHandles = new DWORD[m_nTessLevels];
	for (int i = 0; i < m_nTessLevels; i++)
	{
		BeginStateBlock();
		if ( !SetNPatchMode( m_TessLevels[i][0] ) )
		{
			WriteToLog("CDataTypesNTest::Setup - SetNPatchMode failed on %d.\n", m_TessLevels[i][0] );
			return false;
		}
		if ( !EndStateBlock(&m_dwStateBlockHandles[i]) )
		{
			WriteToLog("CDataTypesNTest::Setup - EndStateBlock failed on %d state block.\n", i);
			return false;
		}
	}
...
void CDataTypesNTest::Cleanup()
{
	...
	if (m_dwStateBlockHandles)
	{
		for (int i = 0; i < m_nTessLevels; i++)
			DeleteStateBlock(m_dwStateBlockHandles[i]);
		delete [] m_dwStateBlockHandles;
		m_dwStateBlockHandles = NULL;
	}
}