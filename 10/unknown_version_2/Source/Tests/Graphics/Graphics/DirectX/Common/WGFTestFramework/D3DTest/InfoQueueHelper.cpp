...
void
CInfoQueueHelper::AddExpectedMessage(D3D10_MESSAGE_ID ID,
...
void
CInfoQueueHelper::AddExpectedMessageString(D3D10_MESSAGE_ID ID,
										   D3D10_MESSAGE_CATEGORY Category,
										   D3D10_MESSAGE_SEVERITY Severity,
										   ...
										   ...)
{
	...
	HRESULT hr = StringCchVPrintf(localBuffer, 2047, pDescription, args);
	...
	if( FAILED( hr ) )
		throw( _T( "StringCchVPrintf() failed in InfoQueueHelper.cppp" ) );
...
bool
CInfoQueueHelper::MarkMessageAsCovered(D3D10_MESSAGE* d3dMsg, INT64 msgIndex)
{
...
CInfoQueueHelper::CheckMessageMatches(D3D10_MESSAGE* d3dMsg,
...
CInfoQueueHelper::FindCoveredMessages(bool bIgnoreUnexpected)
{
	...
	pMsg = (D3D10_MESSAGE *)blob->GetBufferPointer();
...
//Returns D3D10_MESSAGE_ID_UNKNOWN if no more uncovered message id
...
CInfoQueueHelper::LogUncoveredMessages(CTest* test)
{
	...
	for (UINT64 i = 0; i < m_InfoQueueMatches.size(); ++i)
	{
		if (!m_InfoQueueMatches[(size_t)i])
		{
			...
			D3D10_MESSAGE* pMsg = (D3D10_MESSAGE *)blob->GetBufferPointer();
