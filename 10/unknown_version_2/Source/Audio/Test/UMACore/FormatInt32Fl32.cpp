...
//-------------------------------------------------------------------------
// Class: 
// Method: Tc_APOFormatInt32Fl32Create
//
// Description:
//  TestCase to test if AudioFormatInt32Fl32 class is registered and the necessary interfaces can be obtained from it
...
{
    CAPOFormatInt32Fl32Test APOFormatTest;
    return APOFormatTest.APOTestCreate(__uuidof(AudioFormatConvert));
}

//-------------------------------------------------------------------------
// Class: 
// Method: Tc_APOFormatInt32Fl32Process
//
// Description:
//  TestCase to test if APO AudioFormatInt32Fl32 behaves as expected
//  This is a global testcase function used in rgTestCaseInfo structure
// Parameters:
//
// Return values (DWORD):
...
//-------------------------------------------------------------------------
// Class: CAPOFormatInt32Fl32Test
// Method: APOFormatProcess
//
// Description:
//   Functional testcase for FormatInt32Fl32 APO
// Parameters:
//
// Return values (DWORD):
//     Return FNS_PASS on Success, FNS_FAIL on Failure
//
DWORD CAPOFormatInt32Fl32Test::APOFormatProcess()
{
    HRESULT hr = S_OK;
    DWORD dw = FNS_PASS;
    ...
    SLOG(eInfo1, "Creating FormatInt32Fl32 APO");
    hr = pAPORT.CreateInstance(__uuidof(AudioFormatConvert));
    if(FAILED(hr))
    {
        SLOG(eError, "ERROR: APORT Object creation from AudioFormatInt32Fl32 FAILED, 0x%X", hr);
        dw = FNS_FAIL;
        goto exit;
    }
    SLOG(eInfo1, "Querying for IAPOConfig from FormatInt32Fl32");
    ...
    FLOAT32 OutputData[8];
    ZeroMemory(OutputData, sizeof(FLOAT32) * 8);
    ...
    if( 
        ! CCheck::InRange(OutputData[0], FLOAT32(0.0),0.999, 1.001) ||
        ! CCheck::InRange(OutputData[1], FLOAT32(0.0),0.999, 1.001) || 
        ! CCheck::InRange(OutputData[2], FLOAT32(1.0),0.999, 1.001) || 
        ! CCheck::InRange(OutputData[3], FLOAT32(-1.0),0.999, 1.001) || 
        ! CCheck::InRange(OutputData[4], FLOAT32(0.5),0.999, 1.001) || 
        ! CCheck::InRange(OutputData[5], FLOAT32(-0.5),0.999, 1.001) || 
        ! CCheck::InRange(OutputData[6], FLOAT32(4.65661e-10),0.999, 1.001) || 
        ! CCheck::InRange(OutputData[7], FLOAT32(-4.65661e-10),0.999, 1.001)
      )
    {
    ...
    //Start with a INT32 data, Convert it to FLOAT32 and reconvert it to INT32
    ...
    // Call Int32Fl32 collect the data in a temp buffer, then call Fl32Int32 and collect the data in the 
    ...
    //At this point the sinewave has gone through a complete roundtrip from  to FLOAT32 to INT32
    ...
    for (UINT32 i = 0; i < 480; i++)
    {
        ...
        if(! CCheck::InRange(pOutput[i], pSineWave[i], 256))
        {
            SLOG(eError, "ERROR: APO Process did not behave as expected");
    ...
    // THE FOLLOWING WAS WRITTEN TO CHECK THE PERFORMANCE OVER THE ENTIRE DYNAMIC RANGE OF INT32 WHICH 
    // OFCOURSE TAKES A LONG TIME AND SO HAS BEEN COMMENTED OUT.
    // WITH SHELL98 LOGGING THE MEM USAGE WILL ALSO CLIMB VERY STEEPLY, AND THE MACHINE MAY BECOME VERY SLUGGISH

    // UNCOMMENT THE LINES BELOW IF THIS SEEMS INTERESTING ENOUGH.

    ////// Check bit for bit transparency over the entire dynamic range, 
    //////                                             Positive Max, Positive Max/2, Positive 0    
    //////Allocate Source and destination buffers
    //pSineWave = (INT32*)LocalAlloc(LMEM_FIXED, sizeof(INT32) * 65536); //enitre 32 but dynamic range..2^32
    //pOutput = (INT32*)LocalAlloc(LPTR, sizeof(INT32) * 65536); //Zero Out the destination space
    //_ASSERT(pSineWave && pOutput);
    //BUILD_FORMAT(&AudioIntFormat, KSDATAFORMAT_SUBTYPE_PCM, 1, 4, 32, 48000);
    //BUILD_FORMAT(&AudioFloatFormat, KSDATAFORMAT_SUBTYPE_IEEE_FLOAT, 1, 4, 32, 48000);
    //SLOG(eInfo1, "Testing bit for bit transparency for the entire dynamic range of INT32"); 
    //SLOG(eInfo1, "~~~~~~~~~~~~~~~~THIS COULD TAKE A WHILE~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"); 
    //for (UINT j = 0x0000; j <= 0xFFFF; j++)
    //{ //// VERY VERY VERY LONG LOOP.... In every loop we check for 0xFFFF values 
    //    for (UINT i = 0x0000; i <= 0xFFFF; i++)
    //    {
    //        pSineWave[i] = (INT32)(i + (0x10000 * j));
    //    }
    //    dw = APOTestProcess( pSineWave, //Source data buffer
    //        AudioIntFormat, //Source format
    //        __uuidof(AudioFormatInt32Fl32), //Source CLSID
    //        pOutput,  //destination buffer
    //        AudioFloatFormat,  // Intermediate data format
    //        __uuidof(AudioFormatFl32Int32), // Intermediate APO CLSID
    ...
    //    {
    //        if(! CCheck::InRange(pOutput[k], pSineWave[k], 128))
    //        {
    //            SLOG(eError, "ERROR: APO Process did not behave as expected");
    //            SLOG(eError, "Source Value: %d            Dest Value: %d,    Frame no: %d", pSineWave[k], pOutput[k], k);
    //            dw = FNS_FAIL;
    //        }
    //    }
    //} // for (UINT j = 0x0000; j < 0xFFFF; j++)
